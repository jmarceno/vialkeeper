defmodule VialKeeper.Storage.SQLite.ViewsTest do
  ...
  test "view CRUD and name conflict", %{adapter: adapter} do
    ...
    assert {:error, %VialKeeper.Error{code: :view_name_conflict}} =
    ...
    assert {:error, %VialKeeper.Error{code: :view_not_found}} =
             @adapter.view_state(adapter, view_id)
  end
  ...
  test "apply batch rolls back indexed_through when row upsert fails", %{adapter: adapter} do
    ...
    fault_adapter =
      PortFault.inject(
        adapter,
        :view_upsert_row,
        {:once, VialKeeper.Error.internal_error("injected view upsert fault")}
      )
