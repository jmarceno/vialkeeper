defmodule VialKeeper.Contract.StrictDecoderTest do
  @moduledoc """
  ...
  alias VialKeeper.JSON.{StrictCache, StrictDecoder}
  ...
  describe "nesting depth and size (JSON-002, SEC-001)" do
    ...
    test "uses host-configured nesting depth when option omitted" do
      ...
      on_exit(fn ->
        if previous,
          do: Application.put_env(:vial_keeper, :host_limits, previous),
          else: Application.delete_env(:vial_keeper, :host_limits)
      end)

      Application.put_env(:vial_keeper, :host_limits, max_json_nesting_depth: 1)
  ...
  describe "UTF-8 and malformed input (JSON-002)" do
    ...
    test "rejects empty input" do
      assert {:error, %VialKeeper.Error{code: :invalid_request}} = StrictDecoder.decode("")
    end
  end

  describe "fixture-driven reject vectors" do
    test "loads and applies decoder reject fixtures" do
      path = Path.join(:code.priv_dir(:vial_keeper), "fixtures/strict_json/rejects.json")
      ...
      for fixture <- fixtures, fixture["skip"] != true do
        ...
        assert {:error, %VialKeeper.Error{code: ^expected_code, message: message}} =
