defmodule VialKeeper.EndToEnd.OfflineCopyTest do
  ...
  test "offline copy, registration, derived index rebuild, and integrity" do
    source_rel = "e2e-offline-src-#{System.unique_integer([:positive])}.vialkeeper"
    dest_rel = "e2e-offline-dst-#{System.unique_integer([:positive])}.vialkeeper"
    ...
    assert {:ok, %{documents: pre_docs, selected_index: ^structured_id}} =
             VialKeeper.Query.execute(source_uuid, %{
               "selector" => %{"/kind" => "task"},
               "index" => "by-kind",
               "limit" => 10
             })
    ...
    on_exit(fn ->
      ...
      VialKeeper.TempDatabase.cleanup(dest_abs)
    end)

    assert {:ok, %{revision: ^revision, body: %{"copied" => true, "kind" => "task"}}} =
             Documents.get(source_uuid, %{id: "portable"})

    assert {:ok, indexes} = VialKeeper.Query.list_indexes(source_uuid)
    ...
    for %{"index_id" => index_id} <- indexes do
      assert {:ok, %{rebuilt: true}} = VialKeeper.Query.rebuild_index(source_uuid, index_id)
    end
  ...
  test "copied bundle preserves attachments, encodings, and ignores lease files" do
    source_rel = "e2e-offline-att-src-#{System.unique_integer([:positive])}.vialkeeper"
    dest_rel = "e2e-offline-att-dst-#{System.unique_integer([:positive])}.vialkeeper"
    root = VialKeeper.Config.database_root()
    source_abs = Path.join(root, source_rel)
    dest_abs = Path.join(root, dest_rel)

    for path <- [source_abs, dest_abs], do: VialKeeper.TempDatabase.cleanup(path)
  ...
  test "crash after install before commit leaves orphan; after commit never missing blob" do
    rel = "e2e-offline-crash-#{System.unique_integer([:positive])}.vialkeeper"
