defmodule ElixirDB.View.NumericAccumulatorTest do
  ...
  test "subnormal sumsqr below binary64 range returns resource_limit" do
    ...
    assert {:error, %ElixirDB.Error{code: :resource_limit}} = NumericAccumulator.sumsqr(acc)
  end
  ...
  test "overflow returns resource_limit" do
    ...
    assert {:error, %ElixirDB.Error{code: :resource_limit}} = NumericAccumulator.sum(acc)
    assert {:error, %ElixirDB.Error{code: :resource_limit}} = NumericAccumulator.sumsqr(acc)
  end
