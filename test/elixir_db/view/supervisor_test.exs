defmodule ElixirDB.View.SupervisorTest do
  ...
  setup do
    rel = "view-supervisor-#{System.unique_integer([:positive])}.elixirdb"
  ...
  test "view supervisor is registered in runtime child order", %{uuid: uuid} do
    assert [{_pid, _}] =
             Registry.lookup(ElixirDB.Runtime.DatabaseRegistry, {:view_supervisor, uuid})

    assert [{_pid, _}] =
             Registry.lookup(ElixirDB.Runtime.DatabaseRegistry, {:view_manager, uuid})

    assert [{_pid, _}] =
             Registry.lookup(ElixirDB.Runtime.DatabaseRegistry, {:view_builder_supervisor, uuid})
  end
end
