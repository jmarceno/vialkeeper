defmodule VialKeeper.Runtime.AdmissionTest do
  ...
  setup do
    ...
    relative = "admission-#{System.unique_integer([:positive])}.vialkeeper"
  ...
  test "execute_with_deadline fails when absolute deadline is already past", %{uuid: uuid} do
    ...
    assert {:error,
            %VialKeeper.Error{
             ...
             VialKeeper.Query.execute_with_deadline(uuid, %{selector: %{}, limit: 1}, deadline_ms)
  end
