defmodule ElixirDB.Federation.BookmarkCodec do
  ...
  def encode(payload) when is_map(payload) do
    unsigned =
      payload |> Map.put("version", 1) |> Map.put("protocol_major", ElixirDB.protocol_major())
  ...
  def decode(bookmark, expected) when is_binary(bookmark) do
    with {:ok, bytes} <- Base.url_decode64(bookmark, padding: false),
         ...
         true <-
           unsigned["version"] == 1 and unsigned["protocol_major"] == ElixirDB.protocol_major(),
      ...
      _ ->
        {:error,
         ElixirDB.Error.invalid_bookmark("bookmark is invalid or bound to another federation query")}
    end
  end
