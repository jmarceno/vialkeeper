defmodule ElixirDB.Domain.RetentionState do
  ...
  def new(attrs) when is_map(attrs) do
    if Enum.any?(Map.keys(attrs), &(&1 not in @known)),
      do: {:error, ElixirDB.Error.invalid_request("unknown retention state field")},
      else: build(attrs)
  end

  def new(_), do: {:error, ElixirDB.Error.invalid_request("retention state must be an object")}
  ...
  def from_wire(attrs) when is_map(attrs) do
    ...
    if Enum.any?(Map.keys(attrs), &(&1 not in allowed)) do
      {:error, ElixirDB.Error.invalid_request("unknown retention state field")}
    else
  ...
  def from_wire(_),
    do: {:error, ElixirDB.Error.invalid_request("retention state must be an object")}
  ...
  defp validate_history_epoch(_),
    do: ElixirDB.Error.invalid_request("retention state history_epoch is required")
  ...
  defp validate_floor_sequence(_),
    do: ElixirDB.Error.invalid_request("retention floor_sequence must be non-negative")
  ...
  defp validate_compaction_epoch(_),
    do: ElixirDB.Error.invalid_request("retention compaction_epoch must be non-negative")
  ...
  defp validate_boundary_digest(_),
    do: ElixirDB.Error.invalid_request("retention boundary_digest must be a binary or null")

  defp validate_mode(%{mode: value}) when value in [:disabled, :stable_frontier], do: nil

  defp validate_mode(_),
    do: ElixirDB.Error.invalid_request("retention mode must be disabled or stable_frontier")
  ...
  defp validate_maintenance_counter(_),
    do: ElixirDB.Error.invalid_request("retention maintenance_counter must be non-negative")
  ...
  defp decode_mode(_),
    do:
      {:error, ElixirDB.Error.invalid_request("retention mode must be disabled or stable_frontier")}
