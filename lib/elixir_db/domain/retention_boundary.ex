defmodule ElixirDB.Domain.RetentionBoundary do
  ...
  def new(attrs) when is_map(attrs) do
    if Enum.any?(Map.keys(attrs), &(&1 not in @known)),
      do: {:error, ElixirDB.Error.invalid_request("unknown retention boundary field")},
      else: build(attrs)
  end

  def new(_), do: {:error, ElixirDB.Error.invalid_request("retention boundary must be an object")}
  ...
  def from_wire(attrs) when is_map(attrs) do
    ...
    if Enum.any?(Map.keys(attrs), &(&1 not in allowed)) do
      {:error, ElixirDB.Error.invalid_request("unknown retention boundary field")}
    else
  ...
  def from_wire(_),
    do: {:error, ElixirDB.Error.invalid_request("retention boundary must be an object")}
  ...
  defp validate_document_id(_),
    do: ElixirDB.Error.invalid_request("retention boundary document_id is required")

  defp validate_history_id(%{history_id: value}) when is_binary(value) and value != "", do: nil

  defp validate_history_id(_),
    do: ElixirDB.Error.invalid_request("retention boundary history_id is required")

  defp validate_retired_branch_roots(%{retired_branch_roots: value}) when is_list(value) do
    if Enum.all?(value, &(is_binary(&1) and &1 != "")),
      do: nil,
      else:
        ElixirDB.Error.invalid_request("retention boundary retired_branch_roots must be binaries")
  end

  defp validate_retired_branch_roots(_),
    do: ElixirDB.Error.invalid_request("retention boundary retired_branch_roots must be an array")
  ...
  defp validate_retired_state(%{retired: retired}) when not is_boolean(retired),
    do: ElixirDB.Error.invalid_request("retention boundary retired must be boolean")
