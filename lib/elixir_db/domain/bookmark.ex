defmodule ElixirDB.Domain.Bookmark do
  ...
  defp build(attrs) do
    with {:ok, attrs} <- normalize_attrs(attrs),
      ...
      error when is_struct(error, ElixirDB.Error) -> {:error, error}
      {:error, _} = error -> error
    end
  end
  ...
  defp validate_plan_digest(%{plan_digest: value}) when is_binary(value) do
    if valid_digest?(value),
      do: nil,
      else: ElixirDB.Error.invalid_bookmark("bookmark plan_digest is invalid")
  end

  defp validate_plan_digest(_),
    do: ElixirDB.Error.invalid_bookmark("bookmark plan_digest is required")

  defp validate_index_bindings(%{index_bindings: value}) when is_list(value), do: nil

  defp validate_index_bindings(_),
    do: ElixirDB.Error.invalid_bookmark("bookmark index_bindings must be an array")
  ...
  defp validate_ordering_key(%{ordering_key: value}) when is_list(value) do
    if valid_cursor_value?(value),
      do: nil,
      else: ElixirDB.Error.invalid_bookmark("bookmark ordering_key is invalid")
  end
  ...
  defp validate_ordering_key(%{ordering_key: value}) when is_map(value) do
    case normalize_ordering_cursor(value) do
      ...
      _ ->
        ElixirDB.Error.invalid_bookmark("bookmark ordering_key is invalid")
    end
  end

  defp validate_ordering_key(_),
    do: ElixirDB.Error.invalid_bookmark("bookmark ordering_key is required")
    ...
    Enum.reduce_while(bindings, {:ok, [], MapSet.new()}, fn binding, {:ok, acc, ids} ->
      with {:ok, normalized} <- normalize_binding(binding),
        ...
        true ->
          {:halt,
           {:error,
            ElixirDB.Error.invalid_bookmark("bookmark index_bindings must be ordered and unique")}}
  ...
  defp normalize_bindings(_),
    do: {:error, ElixirDB.Error.invalid_bookmark("bookmark index_bindings must be an array")}
  ...
  defp normalize_binding(binding) when is_map(binding) do
    ...
    cond do
      key_collision?(keys) ->
        {:error, ElixirDB.Error.invalid_bookmark("bookmark index binding keys must be unique")}

      Enum.any?(keys, &(&1 not in [:index_id, :definition_digest, "index_id", "definition_digest"])) ->
        {:error,
         ElixirDB.Error.invalid_bookmark("bookmark index binding contains an unknown field")}

      not is_binary(index_id) or index_id == "" ->
        {:error, ElixirDB.Error.invalid_bookmark("bookmark index_id is invalid")}

      not valid_digest?(definition_digest) ->
        {:error, ElixirDB.Error.invalid_bookmark("bookmark definition_digest is invalid")}
  ...
  defp normalize_binding(_),
    do: {:error, ElixirDB.Error.invalid_bookmark("bookmark index binding must be an object")}
