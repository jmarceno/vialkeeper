defmodule ElixirDB.Storage.AdapterFacade do
  @moduledoc """
  Shared compatibility callbacks for storage adapters.

  The runtime uses `ElixirDB.Storage.Services` with an opaque
  `ElixirDB.Storage.BackendContext`; concrete adapters expose these callbacks
  ...
  defp map_callback(name, service, label) do
    quote do
      ...
      def unquote(name)(_adapter, _request),
        do: {:error, ElixirDB.Error.invalid_request(unquote(label) <> " request must be an object")}
    end
  end
