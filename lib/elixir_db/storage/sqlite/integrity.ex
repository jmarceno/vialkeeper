defmodule ElixirDB.Storage.SQLite.Integrity do
  ...
  `ElixirDB.Integrity.Rules` over a normalized snapshot. This module owns
  ...
  defp validate_document_term(body_json, body_term, 0)
    ...
    case TermBlob.decode(body_term, body_json) do
      {:ok, _} -> :ok
      _ -> {:error, ElixirDB.Error.integrity_violation("materialized winner is inconsistent")}
    end
  end

  defp validate_document_term(_body_json, _body_term, 1),
    do: {:error, ElixirDB.Error.integrity_violation("materialized winner is inconsistent")}
  ...
  defp normalize_change_row([sequence, document_id, winning, leaf_json, leaf_term]) do
    with {:ok, leaves} <- StrictDecoder.decode(leaf_json),
      ...
      {:fallback, _} ->
        {:error, ElixirDB.Error.integrity_violation("change JSON and term differ")}
