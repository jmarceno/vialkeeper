defmodule ElixirDB.Storage.Ports.Errors do
  @moduledoc """
  Normalizes backend failures at the storage port edge.

  Callers of storage ports always receive `ElixirDB.Error.t()` values. Engine
  ...
  def normalize(reason),
    do: ElixirDB.Error.internal_error("storage backend operation failed", %{cause: inspect(reason)})
