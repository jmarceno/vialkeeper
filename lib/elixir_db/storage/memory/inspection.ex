defmodule ElixirDB.Storage.Memory.Inspection do
  @moduledoc """
  ...
  `ElixirDB.Integrity.Rules` can run without SQL. Physical probes report the
  memory engine only.
  """
  ...
  alias ElixirDB.Storage.Services.Integrity
  ...
  defp decode_changes(changes) do
    Enum.reduce_while(changes, {:ok, []}, fn change, {:ok, acc} ->
      case StrictDecoder.decode(change.leaf_set_json) do
        ...
        _ ->
          {:halt, {:error, ElixirDB.Error.integrity_violation("change JSON and term differ")}}
      end
    end)
    |> case do
