defmodule ElixirDB.Storage.Memory.DerivedState do
  ...
  orchestration lives in `ElixirDB.Storage.Services.DerivedViews`.
  """
  ...
  defp seed_sources(sources) when is_list(sources) do
    ...
    |> Enum.reduce_while({:ok, %{}}, fn {source, ordinal}, {:ok, acc} ->
      case Engine.source_uuid(source) do
        ...
        _ ->
          {:halt, {:error, ElixirDB.Error.invalid_request("derived source uuid is invalid")}}
      end
    end)
  end

  defp seed_sources(_), do: {:error, ElixirDB.Error.invalid_request("derived sources are invalid")}
