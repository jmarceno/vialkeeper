defmodule ElixirDB.Observability.Instrumentation.Subscription do
  @moduledoc """
  ...
  def open(uuid) when is_binary(uuid) do
    Meters.add(:"elixir_db.query.subscription.open", db_uuid: uuid)
  end
  ...
  def update(uuid, type) when is_binary(uuid) and is_atom(type) do
    Meters.add(:"elixir_db.query.subscription.update", db_uuid: uuid, event_type: type)
  end

  @doc "Records a subscription overload termination."
  @spec overload(binary()) :: :ok
  def overload(uuid) when is_binary(uuid) do
    Meters.add(:"elixir_db.query.subscription.overload", db_uuid: uuid)
  end
end
