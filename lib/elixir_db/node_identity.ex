defmodule ElixirDB.NodeIdentity do
  ...
  @spec ensure(Path.t()) :: {:ok, String.t()} | {:error, ElixirDB.Error.t()}
  def ensure(root) when is_binary(root) do
    ...
    case File.read(path) do
      ...
      {:error, reason} ->
        {:error,
         ElixirDB.Error.database_unavailable("node identity cannot be read", %{
           cause: inspect(reason)
         })}
    end
  end

  @spec get() :: {:ok, String.t()} | {:error, ElixirDB.Error.t()}
  def get, do: ensure(ElixirDB.Config.database_root())
  ...
  defp create(path) do
    ...
    case AtomicWrite.write(path, identity <> "\n") do
      ...
      {:error, reason} ->
        {:error,
         ElixirDB.Error.database_unavailable("node identity cannot be persisted", %{
           cause: inspect(reason)
         })}
    end
  end
  ...
  defp validate(value) do
    ...
    if Regex.match?(@uuid_pattern, identity),
      ...
      else: {:error, ElixirDB.Error.integrity_violation("node identity is invalid")}
  end
end
