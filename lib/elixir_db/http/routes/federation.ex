defmodule ElixirDB.HTTP.Routes.Federation do
    ...
    case Map.get(body, "name") do
      name when is_binary(name) and name != "" ->
        case SavedQueries.get(name) do
          nil -> Response.error(conn, ElixirDB.Error.invalid_request("saved query not found"))
          saved -> Response.result(conn, Federation.query(saved_request(saved, body)))
        end

      _ ->
        Response.error(conn, ElixirDB.Error.invalid_request("saved query name is required"))
    end
  end
