defmodule ElixirDB.Query.PrefixBounds do
  @moduledoc "Safe Unicode-scalar bounds for exact string-prefix candidates."

  @type t :: %{lower: binary(), upper: binary() | nil}

  @spec bounds(binary()) :: {:ok, t()} | {:error, ElixirDB.Error.t()}
  def bounds(prefix) when is_binary(prefix) do
    cond do
      prefix == "" ->
        {:error, ElixirDB.Error.invalid_request("prefix must be non-empty")}

      not String.valid?(prefix) ->
        {:error, ElixirDB.Error.invalid_request("prefix must be valid UTF-8")}
  ...
  def bounds(_), do: {:error, ElixirDB.Error.invalid_request("prefix must be a string")}

  @doc "Alias for `bounds/1`."
  @spec calculate(binary()) :: {:ok, t()} | {:error, ElixirDB.Error.t()}
