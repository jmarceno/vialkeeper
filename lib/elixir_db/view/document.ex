defmodule ElixirDB.View.Document do
  ...
  alias ElixirDB.View.{Definition, Expression}
  ...
  def map(_definition, _document_id, _revision_id, _body),
    do: {:error, ElixirDB.Error.invalid_request("document body must be an object")}

  defp evaluate_row(definition, document_id, revision_id, body) do
    with {:ok, key} <- evaluate_key(definition.key, body),
         :ok <- ensure_key(key),
