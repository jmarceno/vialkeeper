defmodule ElixirDB.View.Builder do
  @moduledoc """
  ...
  alias ElixirDB.Observability.Instrumentation.View, as: ViewInstrumentation
  alias ElixirDB.Runtime.{ChangeNotifier, ChildSpec, DatabaseCatalog}
  alias ElixirDB.Storage.Results.ReadChanges
  alias ElixirDB.View.{Definition, Document}
  ...
  def via(uuid, view_id),
    do: {:via, Registry, {ElixirDB.Runtime.DatabaseRegistry, {:view_builder, uuid, view_id}}}
  ...
  defp await_sequence_loop(uuid, view_id, target, deadline) do
    ...
    if remaining <= 0 do
      {:error,
       ElixirDB.Error.view_not_caught_up("view did not catch up before the wait deadline", %{
      ...
      case Registry.lookup(ElixirDB.Runtime.DatabaseRegistry, {:view_builder, uuid, view_id}) do
        [{pid, _}] ->
          try do
            ...
            :exit, {:timeout, _} ->
              {:error,
               ElixirDB.Error.view_not_caught_up(
  ...
  @spec request_rebuild(binary(), binary()) :: :ok | {:error, ElixirDB.Error.t()}
  ...
  def handle_info(:bootstrap, state) do
    case load_context(state) do
      ...
      {:error, %ElixirDB.Error{code: :view_not_found}} ->
        {:stop, :normal, state}
  ...
  defp apply_incremental_batch(state, cursor, batch) do
    ...
    with {:ok, rows, removals} <- envelopes_to_rows(state, batch),
      ...
      {:error, %ElixirDB.Error{code: :revision_conflict}} ->
  ...
  defp fetch_view_entry(state) do
    with {:ok, views} <- maintenance(state.uuid, {:command, :list_views, %{}}) do
      case Enum.find(views, &(&1["view_id"] == state.view_id)) do
        nil ->
          {:error, ElixirDB.Error.view_not_found("view was not found", %{view_id: state.view_id})}
  ...
  defp decode_definition(view) when is_map(view) do
    case Map.get(view, "definition") || Map.get(view, "definition_json") do
      ...
      _ -> {:error, ElixirDB.Error.internal_error("view definition is missing")}
    end
  end
  ...
  defp probe(state, event, metadata) do
    case Application.get_env(:elixir_db, :view_builder_probe) do
