defmodule ElixirDB.Shadow.Observation do
  ...
  def new(_), do: {:error, ElixirDB.Error.invalid_request("shadow observation must be an object")}
  ...
  defp normalize_state(value) when is_binary(value) do
    ...
    if atom in @states,
      do: {:ok, atom},
      else: {:error, ElixirDB.Error.invalid_request("unknown shadow lifecycle state")}
  rescue
    ArgumentError -> {:error, ElixirDB.Error.invalid_request("unknown shadow lifecycle state")}
  end

  defp normalize_state(_),
    do: {:error, ElixirDB.Error.invalid_request("unknown shadow lifecycle state")}
