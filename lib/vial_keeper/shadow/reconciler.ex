defmodule VialKeeper.Shadow.Reconciler do
  ...
  alias VialKeeper.Shadow.Reconciler.EndpointCapabilities
  ...
  defp notify(source_uuid, message) do
    case ControllerRegistry.lookup(VialKeeper.Shadow.ControllerRegistry, source_uuid) do
      [{pid, _}] ->
  ...
  defp start_or_notify(%Definition{} = definition) do
    case ControllerRegistry.lookup(VialKeeper.Shadow.ControllerRegistry, definition.source_uuid) do
      [{pid, _}] ->
  ...
  defp controller_enabled?,
    do: Keyword.get(Application.get_env(:vial_keeper, :shadow_controller, []), :enabled, false)

  @impl true
  def init(%{definition: %Definition{} = definition} = opts) do
    case ControllerRegistry.register(
           VialKeeper.Shadow.ControllerRegistry,
  ...
  def handle_info({:DOWN, ref, :process, _pid, reason}, %{task: %Task{ref: ref}} = state) do
    observe_error(
      state.definition,
      VialKeeper.Error.database_unavailable("shadow control task stopped", %{cause: reason})
    )
  ...
  defp configured_endpoint(definition) do
    locations = Application.get_env(:vial_keeper, :shadow_controller, [])[:location] || []
    ...
    case Enum.find(locations, &(&1[:name] == definition.location)) do
      ...
      nil -> {:error, VialKeeper.Error.invalid_request("shadow location is not configured")}
    end
  end

  defp provision_request(definition) do
    source = Application.get_env(:vial_keeper, :shadow_controller, [])
  ...
  defp destroy(definition, options) do
    case endpoint(definition, options) do
      ...
      {:error, _} ->
        {:error, VialKeeper.Error.invalid_request("shadow location is not configured")}
    end
  end
  ...
  defp observe_error(definition, _error),
    do: observe_error(definition, VialKeeper.Error.internal_error())

  defp control_timeout(%LocalEndpoint{}), do: VialKeeper.Config.request_timeout_ms()
  defp control_timeout(%RemoteEndpoint{control_timeout_ms: timeout}), do: timeout
  defp control_timeout(_), do: VialKeeper.Config.request_timeout_ms()
