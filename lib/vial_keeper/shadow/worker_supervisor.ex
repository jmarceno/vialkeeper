defmodule VialKeeper.Shadow.WorkerSupervisor do
  ...
  def start_worker(opts),
    do: DynamicSupervisor.start_child(__MODULE__, {VialKeeper.Shadow.Worker, opts})
  ...
  def start_replicator(request, opts \\ []) when is_map(request) do
    DynamicSupervisor.start_child(
      __MODULE__,
      {VialKeeper.Shadow.Replicator, Keyword.put(opts, :request, request)}
    )
  end
