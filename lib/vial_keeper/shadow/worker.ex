defmodule VialKeeper.Shadow.Worker do
  ...
  defp maybe_start_replicator(request, state) do
    auto_replicate? =
      Keyword.get(
        state.options,
        :auto_replicate,
        Keyword.get(Application.get_env(:vial_keeper, :shadow_worker, []), :auto_replicate, false)
      )
  ...
  defp bulk_requests(requests) when is_list(requests) do
    max_operations = VialKeeper.Config.host_limits()[:max_bulk_operations] || 500
  ...
  defp source_origins(state) do
    Keyword.get(state.options, :allowed_source_origins) ||
      Application.get_env(:vial_keeper, :shadow_worker, [])[:allowed_source_origins] ||
      []
  end
  ...
  defp managed_path(root, request) do
    ...
    filename = "#{request["generation"]}-#{request["shadow_uuid"]}.shadow.vialkeeper"
  ...
  defp root(opts) do
    ...
    case configured do
      ...
      _ ->
        worker = Application.get_env(:vial_keeper, :shadow_worker, [])

        Path.expand(
          Keyword.get(worker, :storage_root, "shadows"),
  ...
  defp attachment_roots(state) do
    Keyword.get(state.options, :allowed_attachment_roots) ||
      Application.get_env(:vial_keeper, :shadow_worker, [])[:allowed_attachment_roots] ||
      []
  end
  ...
  defp catalog_timeout, do: VialKeeper.Config.request_timeout_ms()
