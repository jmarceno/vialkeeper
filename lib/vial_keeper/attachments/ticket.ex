defmodule VialKeeper.Attachments.Ticket do
  ...
  def build(
    ...
    with {:ok, digest} <- Manifest.validate_digest(blob_digest),
      ...
      false -> {:error, VialKeeper.Error.invalid_request("invalid attachment ticket fields")}
      {:error, _} = error -> error
    end
  end
  ...
  def new(attrs) when is_map(attrs) do
    ...
    if Enum.all?(required, &Map.has_key?(attrs, &1)) do
      ...
      {:error, VialKeeper.Error.invalid_request("invalid attachment ticket fields")}
    end
  end

  def new(_), do: {:error, VialKeeper.Error.invalid_request("invalid attachment ticket fields")}
end
