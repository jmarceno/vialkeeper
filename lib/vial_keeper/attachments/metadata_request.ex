defmodule VialKeeper.Attachments.MetadataRequest do
  @moduledoc """
  ...
  @spec request_digest(map()) :: {:ok, binary()} | {:error, VialKeeper.Error.t()}
  ...
  @spec request_logical_size(map()) :: {:ok, non_neg_integer()} | {:error, VialKeeper.Error.t()}
  def request_logical_size(request) when is_map(request) do
    ...
    if is_integer(size) and size >= 0,
      do: {:ok, size},
        ...
        {:error, VialKeeper.Error.invalid_request("logical_size must be a non-negative integer")}
  end

  @doc "Validates every digest in a list."
  @spec validate_digest_list([term()]) :: :ok | {:error, VialKeeper.Error.t()}
  ...
  @spec validate_after_digest(term()) :: :ok | {:error, VialKeeper.Error.t()}
  ...
  def validate_after_digest(digest) when is_binary(digest) do
    if Regex.match?(@digest_pattern, digest),
      do: :ok,
      else: {:error, VialKeeper.Error.invalid_request("after_digest must be lowercase SHA-256 hex")}
  end

  def validate_after_digest(_),
    do: {:error, VialKeeper.Error.invalid_request("after_digest must be lowercase SHA-256 hex")}

  @doc "Resolves optional `now` override to a second-truncated UTC DateTime."
  @spec cleanup_now(map()) :: {:ok, DateTime.t()} | {:error, VialKeeper.Error.t()}
    ...
    case MapAccess.get(request, :now) do
      ...
      value when is_binary(value) ->
        case DateTime.from_iso8601(value) do
          ...
          _ -> {:error, VialKeeper.Error.invalid_request("now must be an RFC3339 timestamp")}
        end

      _ ->
        {:error, VialKeeper.Error.invalid_request("now must be an RFC3339 timestamp")}
    end
  end
  ...
  def resolve_revision_id(_doc, _revision_id),
    do: {:error, VialKeeper.Error.invalid_request("revision must be a string or null")}
