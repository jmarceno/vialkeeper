defmodule VialKeeper.HostConfig do
  @moduledoc """
  ...
  @default_limits %{
    ...
    "max_search_rebuild_ms" => 300_000,
    "max_search_rebuild_batch_documents" => 500,
    "max_search_writer_memory_bytes" => 50_000_000,
  ...
  @limit_key_atoms %{
    ...
    "max_search_rebuild_ms" => :max_search_rebuild_ms,
    "max_search_rebuild_batch_documents" => :max_search_rebuild_batch_documents,
    "max_search_writer_memory_bytes" => :max_search_writer_memory_bytes,
  ...
  @doc """
  Resolves the configured database root from the `VIAL_KEEPER_ROOT` environment
  variable, defaulting to `./data` relative to the working directory.
  """
  @spec database_root :: String.t()
  def database_root do
    case System.get_env("VIAL_KEEPER_ROOT") do
  ...
  `{:ok, keyword}` (a `:vial_keeper` config keyword list) or `{:error, reason}`
  ...
  defp template_path do
    :code.priv_dir(:vial_keeper) |> Path.join(@filename)
  end
  ...
  # Validation produces a flat `:vial_keeper` keyword list.
  defp validate(raw, root) do
    with :ok <- validate_sections(raw),
      ...
      # host_limits consumers (`VialKeeper.Config.host_limits/0`) expect atom keys,
  ...
  defp validate_path_inside_root(path, root, _field) when is_binary(path) do
    ...
    if String.starts_with?(resolved <> "/", root_abs <> "/") and
         VialKeeper.PathSafety.no_symlink_components?(resolved) do
      :ok
    else
