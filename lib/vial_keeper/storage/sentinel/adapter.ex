defmodule VialKeeper.Storage.Sentinel.Adapter do
  @moduledoc """
  ...
  alias VialKeeper.Storage.Sentinel.{Ownership, Transaction}
  ...
  def port_modules do
    %{
      lifecycle: VialKeeper.Storage.Sentinel.Lifecycle,
      transaction: VialKeeper.Storage.Sentinel.Transaction,
      ownership: VialKeeper.Storage.Sentinel.OwnershipPort
    }
  end
  ...
  defp ensure_root(path) do
    case File.mkdir_p(path) do
      ...
      {:error, reason} ->
        {:error, VialKeeper.Error.internal_error("cannot create sentinel root: #{inspect(reason)}")}
    end
  end
  ...
  defp ensure_existing_root(path) do
    ...
    if File.dir?(path) and File.regular?(marker) do
      ...
      {:error, VialKeeper.Error.invalid_request("sentinel backend marker is missing")}
    end
  end
  ...
  defp write_marker(path, uuid) do
    case File.write(marker_path(path), uuid) do
      ...
      {:error, reason} ->
        {:error,
         VialKeeper.Error.internal_error("cannot write sentinel marker: #{inspect(reason)}")}
    end
  end
  ...
  defp unsupported(operation) do
    {:error, VialKeeper.Error.invalid_request("sentinel backend does not implement #{operation}")}
  end
end
