defmodule VialKeeper.Storage.Memory.IndexCandidates do
  @moduledoc """
  Memory index and candidate port.

  Provides unordered bounded-scan candidates with no structured or full-text
  indexes. Shared `VialKeeper.Query.Executor` owns filtering and product order.
  """
  ...
  def create_index(%BackendContext{}, _definition),
    do: {:error, VialKeeper.Error.invalid_request("memory backend indexes are unsupported")}
  ...
  def range_scan_candidates(%BackendContext{}, _request),
    do:
      {:error, VialKeeper.Error.invalid_index_hint("memory backend has no structured indexes", %{})}

  @impl true
  def full_text_candidates(%BackendContext{}, _request),
    do:
      {:error, VialKeeper.Error.invalid_index_hint("memory backend has no full-text indexes", %{})}
