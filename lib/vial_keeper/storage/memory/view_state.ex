defmodule VialKeeper.Storage.Memory.ViewState do
  @moduledoc """
  ...
  in `VialKeeper.Storage.Services.Views`. Row scans shuffle before filter and sort
  so query order does not depend on map iteration.
  """
  @behaviour VialKeeper.Storage.Ports.ViewState
  ...
  alias VialKeeper.View.{Engine, KeyCodec}
  ...
  def adapter_query_config(%BackendContext{} = context) do
    case Context.unwrap(context) do
      ...
      {:error, _} -> VialKeeper.Config.defaults()
    end
  end
  ...
  defp fetch_view(state, view_id) do
    case Map.fetch(state.views, view_id) do
      {:ok, view} -> {:ok, view}
      :error -> {:error, VialKeeper.Error.view_not_found("view was not found", %{view_id: view_id})}
    end
  end
