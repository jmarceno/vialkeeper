defmodule VialKeeper.Storage.Memory.RetentionRecords do
  @moduledoc """
  ...
  """
  @behaviour VialKeeper.Storage.Ports.RetentionRecords
  ...
  def retention_state(%BackendContext{} = _context) do
    {:error, VialKeeper.Error.invalid_request("use VialKeeper.Storage.Services.retention_state/1")}
  end
  ...
  def read_boundary_pages(%BackendContext{} = _context, _request) do
    {:error,
     VialKeeper.Error.invalid_request("use VialKeeper.Storage.Services.read_boundary_pages/2")}
  end

  @impl true
  def install_boundary_pages(%BackendContext{} = _context, _request) do
    {:error,
     VialKeeper.Error.invalid_request("use VialKeeper.Storage.Services.install_boundary_pages/2")}
  end
  ...
  def put_peer_position_record(%BackendContext{} = context, request) when is_map(request) do
    ...
    with {:ok, adapter} <- Context.unwrap(context),
      ...
      false -> {:error, VialKeeper.Error.invalid_request("peer position CAS fields are invalid")}
  ...
  defp normalize_one(raw) do
    with {:ok, boundary} <-
      ...
      _ -> {:error, VialKeeper.Error.invalid_request("purged boundary is invalid")}
    end
  end
