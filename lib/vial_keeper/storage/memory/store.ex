defmodule VialKeeper.Storage.Memory.Store do
  @moduledoc """
  ...
  alias VialKeeper.Domain.{BoundaryPage, PeerPosition, Revision}
  alias VialKeeper.JSON.Canonical
  alias VialKeeper.MapAccess
  alias VialKeeper.Retention.Service
  ...
  @type change_entry :: %{
          ...
          required(:winner) => VialKeeper.Domain.Revision.t(),
  ...
  @type state :: %{
          ...
          shadow_watermark: non_neg_integer(),
          closed?: boolean()
        }
  ...
  @spec open(binary()) :: {:ok, pid()} | {:error, VialKeeper.Error.t()}
  def open(root) when is_binary(root) do
    ...
    with {:ok, binary} <- File.read(path),
      ...
      {:error, :enoent} ->
        {:error, VialKeeper.Error.invalid_request("memory backend snapshot is missing")}

      {:error, %VialKeeper.Error{} = error} ->
        {:error, error}

      {:error, reason} ->
        {:error, VialKeeper.Error.internal_error("cannot open memory store: #{inspect(reason)}")}
    end
  end

  @doc "Persists and stops the store."
  @spec close(pid()) :: :ok | {:error, VialKeeper.Error.t()}
  ...
  @spec update(pid(), (state() -> {:ok, state(), term()} | {:error, VialKeeper.Error.t()})) ::
  ...
  @spec insert_document_with_revision(state(), binary(), Revision.t(), non_neg_integer()) ::
          {:ok, state(), map()} | {:error, VialKeeper.Error.t()}
  def insert_document_with_revision(state, document_id, %Revision{} = revision, sequence)
    ...
    if Map.has_key?(state.documents, document_id) do
      {:error, VialKeeper.Error.integrity_violation("document already exists")}
    else
  ...
  @spec insert_revision(state(), binary(), Revision.t()) ::
          {:ok, state()} | {:error, VialKeeper.Error.t()}
  def insert_revision(state, document_id, %Revision{} = revision) do
    ...
    if Map.has_key?(by_doc, revision.revision_id) do
      {:error, VialKeeper.Error.integrity_violation("revision already exists")}
    else
      by_doc =
        by_doc
  ...
  @spec empty_document(state(), binary()) :: {:ok, state()} | {:error, VialKeeper.Error.t()}
  ...
  @spec apply_compaction_effect(state(), map()) :: {:ok, state()} | {:error, VialKeeper.Error.t()}
  def apply_compaction_effect(state, effect) when is_map(effect) do
    ...
    KeyError ->
      {:error, VialKeeper.Error.invalid_request("compaction effect is incomplete")}
  end
  ...
  def put_peer_cas(_state, _peer_uuid, _expected, _value),
    do: {:error, VialKeeper.Error.invalid_request("peer position CAS fields are invalid")}
  ...
  defp empty_documents(state, document_ids) do
    Enum.reduce_while(document_ids, {:ok, state}, fn document_id, {:ok, acc} ->
      case empty_document(acc, document_id) do
        {:ok, new_state} -> {:cont, {:ok, new_state}}
        {:error, %VialKeeper.Error{code: :document_not_found}} -> {:cont, {:ok, acc}}
  ...
  defp validate_peer_cas(expected, observed, current, json)
    ...
    if Canonical.encode!(current.value) == json do
      ...
      {:error,
       VialKeeper.Error.checkpoint_conflict("local record version is stale", %{
  ...
  defp next_peer_version(expected, observed, current, json)
    ...
    if Canonical.encode!(current.value) == json,
      do: {:ok, observed, true},
      else: {:error, VialKeeper.Error.checkpoint_conflict("local record version is stale")}
  end
  ...
  defp next_peer_version(_expected, _observed, _current, _json),
    do: {:error, VialKeeper.Error.checkpoint_conflict("local record version is stale")}
  ...
  defp persist(state) do
    ...
    case File.write(path, :erlang.term_to_binary(%{state | closed?: true})) do
      ...
      {:error, reason} ->
        {:error, VialKeeper.Error.internal_error("cannot persist memory store: #{inspect(reason)}")}
    end
  end
  ...
  defp decode_snapshot(binary) do
    ...
    if is_map(state) and Map.has_key?(state, :identity) do
      ...
      {:error, VialKeeper.Error.invalid_request("memory backend snapshot is invalid")}
    end
  rescue
