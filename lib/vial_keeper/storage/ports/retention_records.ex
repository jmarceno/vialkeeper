defmodule VialKeeper.Storage.Ports.RetentionRecords do
  @moduledoc """
  ...
  @callback put_compaction_result(BackendContext.t(), map()) :: :ok | {:error, VialKeeper.Error.t()}
  ...
  @callback mark_pending_local_causal(BackendContext.t()) :: :ok | {:error, VialKeeper.Error.t()}
  @callback pending_local_causal?(BackendContext.t()) :: result(boolean())
  @callback encode_stored_boundary(map()) :: map()
  @callback apply_compaction_effect(BackendContext.t(), map()) ::
              :ok | {:error, VialKeeper.Error.t()}
