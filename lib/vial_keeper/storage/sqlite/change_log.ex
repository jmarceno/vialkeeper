defmodule VialKeeper.Storage.SQLite.ChangeLog do
  @moduledoc """
  SQLite change-log fact port.
  """
  @behaviour VialKeeper.Storage.Ports.ChangeLog
  ...
  def append_change(%BackendContext{} = context, entry) when is_map(entry) do
    with {:ok, adapter} <- Context.unwrap(context),
      ...
      _ ->
        {:error, VialKeeper.Error.invalid_request("change log entry fields are invalid")}
    end
  end
  ...
  def append_changes(%BackendContext{} = context, entries) when is_list(entries) do
    with {:ok, adapter} <- Context.unwrap(context),
      ...
      _ ->
        {:error, VialKeeper.Error.invalid_request("change log entries are invalid")}
    end
  end
  ...
  defp prepare_entry(_entry),
    do: {:error, VialKeeper.Error.invalid_request("change log entry must be an object")}
end
