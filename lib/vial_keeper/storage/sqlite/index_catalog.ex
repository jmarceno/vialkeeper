defmodule VialKeeper.Storage.SQLite.IndexCatalog do
  @moduledoc """
  ...
  """
  @spec create_tx(Connection.handle(), map()) :: {:ok, map()} | {:error, VialKeeper.Error.t()}
  ...
  """
  @spec delete_tx(Connection.handle(), binary()) :: {:ok, map()} | {:error, VialKeeper.Error.t()}
  ...
  """
  @spec rebuild_tx(Connection.handle(), binary()) :: {:ok, map()} | {:error, VialKeeper.Error.t()}
  ...
  defp find_by_name(conn, name, definition_json, digest_value) do
    case Connection.query(
      ...
      {:ok, [[_id, _existing_digest, _existing_json, _state]]} ->
        {:error,
         VialKeeper.Error.index_name_conflict("index name is already used by another definition", %{
           name: name,
