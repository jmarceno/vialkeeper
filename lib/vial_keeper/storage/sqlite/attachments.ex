defmodule VialKeeper.Storage.SQLite.Attachments do
  @moduledoc """
  ...
  TTL, and reachability policy live in `VialKeeper.Storage.Services.Attachments`.
  Mutation orchestration stays in `Mutations`; revision-row SQL stays in
  `Revisions`.
  """

  alias VialKeeper.Attachments.{Manifest, MetadataRequest, Orchestration, Ticket}
  ...
  """
  @spec resolve_attachment_ticket(map(), map()) :: {:ok, map()} | {:error, VialKeeper.Error.t()}
  ...
  def resolve_attachment_ticket(_adapter, _request),
    do: {:error, VialKeeper.Error.invalid_request("attachment ticket request must be an object")}
    ...
    case Connection.query(
      ...
      {:ok, []} ->
        case Connection.query(
          ...
          {:ok, []} ->
            {:error, VialKeeper.Error.attachment_blob_not_found("attachment blob not found")}
  ...
  def put_pending_blob(conn, row) when is_map(row) do
    ...
    with {:ok, digest} <- Manifest.validate_digest(digest),
      ...
      false ->
        {:error, VialKeeper.Error.invalid_request("pending blob row is incomplete")}
  ...
  @spec list_pending_blobs(Connection.handle()) :: {:ok, [map()]} | {:error, VialKeeper.Error.t()}
  ...
  defp find_document(_conn, _document_id),
    do: {:error, VialKeeper.Error.invalid_request("document id must be a non-empty UTF-8 string")}
    ...
    case Connection.query(
      ...
      {:ok, []} ->
        case revision_exists?(conn, doc_key, revision_id) do
          true ->
            {:error, VialKeeper.Error.attachment_not_found("attachment not found")}
