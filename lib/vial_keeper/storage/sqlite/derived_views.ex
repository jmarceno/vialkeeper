defmodule VialKeeper.Storage.SQLite.DerivedViews do
  @moduledoc """
  ...
  `VialKeeper.Storage.Services.DerivedViews`.
  """
  ...
  @spec get_metadata(Connection.handle()) :: {:ok, map()} | {:error, VialKeeper.Error.t()}
  def get_metadata(conn) do
    case Connection.query(
      ...
      {:ok, []} ->
        {:error, VialKeeper.Error.integrity_violation("derived database metadata is missing")}
  ...
  @spec list_sources(Connection.handle()) :: {:ok, [map()]} | {:error, VialKeeper.Error.t()}
  ...
  @spec get_source(Connection.handle(), binary()) :: {:ok, map()} | {:error, VialKeeper.Error.t()}
  ...
  @spec put_enabled(Connection.handle(), map()) :: {:ok, map()} | {:error, VialKeeper.Error.t()}
  ...
  @spec put_rebuild_cursor(Connection.handle(), map()) :: :ok | {:error, VialKeeper.Error.t()}
  ...
  @spec put_source_checkpoint(Connection.handle(), map()) :: :ok | {:error, VialKeeper.Error.t()}
  ...
  @spec clear_source_error(Connection.handle(), binary()) :: :ok | {:error, VialKeeper.Error.t()}
  ...
  @spec refresh_status(Connection.handle()) :: :ok | {:error, VialKeeper.Error.t()}
  ...
  def fetch_group(conn, group_sort, group_json)
    ...
    case Connection.query(
      ...
      {:ok, _rows} ->
        {:error, VialKeeper.Error.integrity_violation("derived group state is invalid")}
  ...
  @spec upsert_group(Connection.handle(), map()) :: :ok | {:error, VialKeeper.Error.t()}
  ...
  @spec delete_group(Connection.handle(), binary()) :: :ok | {:error, VialKeeper.Error.t()}
  ...
  @spec list_group_numeric_values(Connection.handle(), binary()) ::
          {:ok, [{term(), binary() | nil, binary(), binary()}]} | {:error, VialKeeper.Error.t()}
  ...
  defp decode_numeric_value(json) do
    case StrictDecoder.decode(json) do
      {:ok, value} -> {:ok, value}
      _ -> {:error, VialKeeper.Error.integrity_violation("derived contribution value is invalid")}
    end
  end
  ...
  defp decode_json_array(json) when is_binary(json) do
    case StrictDecoder.decode(json) do
      ...
      _ -> {:error, VialKeeper.Error.integrity_violation("derived contribution key is invalid")}
    end
  end

  defp decode_json_array(_),
    do: {:error, VialKeeper.Error.integrity_violation("derived contribution key is missing")}
  ...
  defp validate_group_json(_stored_json, _expected_json),
    do: {:error, VialKeeper.Error.integrity_violation("derived group key is inconsistent")}
    ...
    case Integer.parse(value) do
      {integer, ""} ->
        if predicate.(integer),
          do: {:ok, integer},
          else: {:error, VialKeeper.Error.integrity_violation("derived group #{field} is invalid")}

      _ ->
        {:error, VialKeeper.Error.integrity_violation("derived group #{field} is invalid")}
    end
  end
  ...
  defp parse_group_integer(value, _field, predicate)
    ...
    if predicate.(value),
      do: {:ok, value},
      else: {:error, VialKeeper.Error.integrity_violation("derived group count is invalid")}
  end

  defp parse_group_integer(_value, field, _predicate),
    do: {:error, VialKeeper.Error.integrity_violation("derived group #{field} is invalid")}
  ...
  defp normalize_error(reason),
    do:
      VialKeeper.Error.internal_error("derived sqlite persistence failed", %{cause: inspect(reason)})
end
