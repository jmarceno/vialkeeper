defmodule VialKeeper.Storage.SQLite.RetentionRecords do
  @moduledoc """
  ...
  * `"replication_state"` — key `"pending_local_causal"` tracks unacknowledged local
  ...
  alias VialKeeper.Domain.{BoundaryPage, PeerPosition, RetentionBoundary}
  ...
  @spec list_peers(Connection.handle()) ::
          {:ok, [PeerPosition.t()]} | {:error, VialKeeper.Error.t()}
  ...
  @spec list_boundaries(Connection.handle(), keyword()) ::
          {:ok, [stored_boundary()]} | {:error, VialKeeper.Error.t()}
  ...
  defp boundary_from_value(_),
    do: {:error, VialKeeper.Error.integrity_violation("retention boundary record is invalid")}
  ...
  @spec boundary_install_state(Connection.handle(), binary()) ::
          {:ok, boundary_install_state() | nil} | {:error, VialKeeper.Error.t()}
  ...
  def begin_boundary_install(conn, install_id, state)
    ...
    KeyError ->
      {:error, VialKeeper.Error.invalid_request("boundary install state is incomplete")}
  end
  ...
  def complete_boundary_install(conn, install_id) when is_binary(install_id) do
    with {:ok, metadata} <- staging_metadata(conn, install_id),
      ...
      false ->
        {:error, VialKeeper.Error.boundary_conflict("installed boundary digest mismatch")}
  ...
  defp decode_boundary_install_state(_, _),
    do: {:error, VialKeeper.Error.integrity_violation("boundary install state is invalid")}

  defp staging_metadata(conn, install_id) do
    case fetch_local_record(conn, @boundary_staging, staging_metadata_key(install_id)) do
      {:ok, nil} ->
        {:error, VialKeeper.Error.boundary_conflict("boundary install session is missing")}
  ...
  defp decode_staging_metadata(%{
    ...
    if is_binary(source_uuid) and source_uuid != "" and is_binary(source_epoch) and
      ...
      {:error, VialKeeper.Error.integrity_violation("boundary install session is invalid")}
    end
  end

  defp decode_staging_metadata(_value),
    do: {:error, VialKeeper.Error.integrity_violation("boundary install session is invalid")}
  ...
  defp replace_authoritative_boundaries(conn, state, boundaries) do
    ...
    KeyError -> {:error, VialKeeper.Error.invalid_request("boundary install state is incomplete")}
  end
  ...
  defp normalize_stored_boundary(%{
    ...
    with true <- is_binary(source_uuid) and source_uuid != "",
      ...
      false -> {:error, VialKeeper.Error.invalid_request("purged boundary metadata is invalid")}
      {:error, error} -> {:error, error}
    end
  end
  ...
  defp normalize_stored_boundary(_),
    do: {:error, VialKeeper.Error.invalid_request("purged boundary metadata is invalid")}
  ...
  defp fetch_pending_local_causal(conn) do
    case fetch_replication_state(conn, "pending_local_causal") do
      ...
      {:ok, _} ->
        {:error, VialKeeper.Error.integrity_violation("pending local causal flag is invalid")}
  ...
  @spec put_last_result(Connection.handle(), map()) :: :ok | {:error, VialKeeper.Error.t()}
  ...
  defp delete_document_revisions(conn, document_id, revision_ids) do
    case doc_key_for(conn, document_id) do
      ...
      {:error, %VialKeeper.Error{code: :document_not_found}} -> {:cont, :ok}
      {:error, error} -> {:halt, {:error, error}}
    end
  end
