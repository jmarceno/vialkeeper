defmodule VialKeeper.Storage.SQLite.Documents do
  @moduledoc """
  ...
  def load_winner(_conn, nil),
    do: {:error, VialKeeper.Error.invalid_request("document_id is required")}
  ...
  @spec find_many(Connection.handle(), [binary()]) ::
          {:ok, %{optional(binary()) => map() | nil}} | {:error, VialKeeper.Error.t()}
  ...
  @spec list_page(Connection.handle(), binary() | nil, pos_integer()) ::
          {:ok, {list(binary()), binary() | nil}} | {:error, VialKeeper.Error.t()}
  ...
  defp winner_body(_body_json, _body_term),
    do: {:error, VialKeeper.Error.integrity_violation("winning document body is missing")}
