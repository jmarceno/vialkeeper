defmodule VialKeeper.Storage.SQLite.RetentionRecordsPort do
  @moduledoc """
  ...
  alias VialKeeper.Storage.SQLite.{Context, LocalRecords, RetentionRecords}

  @impl true
  def retention_state(%BackendContext{} = _context) do
    {:error, VialKeeper.Error.invalid_request("call VialKeeper.Storage.Services.retention_state/1")}
  end
  ...
  def read_boundary_pages(%BackendContext{} = _context, request) when is_map(request) do
    ...
    {:error,
     VialKeeper.Error.invalid_request("call VialKeeper.Storage.Services.read_boundary_pages/2")}
  end
  ...
  def install_boundary_pages(%BackendContext{} = _context, request) when is_map(request) do
    ...
    {:error,
     VialKeeper.Error.invalid_request("call VialKeeper.Storage.Services.install_boundary_pages/2")}
  end
