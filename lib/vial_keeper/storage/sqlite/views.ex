defmodule VialKeeper.Storage.SQLite.Views do
  @moduledoc """
  ...
  bookmarks, and result shaping — lives in `VialKeeper.Storage.Services.Views`.
  """

  alias VialKeeper.JSON.{Canonical, StrictDecoder}
  alias VialKeeper.MapAccess
  alias VialKeeper.Storage.SQLite.{Connection, ProcessResultCache, TermBlob}
  alias VialKeeper.UUID
  alias VialKeeper.View.KeyCodec

  @list_cache_key :vial_keeper_sqlite_view_catalog

  @spec list(Connection.handle()) :: {:ok, [map()]} | {:error, VialKeeper.Error.t()}
  @spec list(Connection.handle(), keyword()) :: {:ok, [map()]} | {:error, VialKeeper.Error.t()}
  ...
  @spec count(Connection.handle()) :: {:ok, non_neg_integer()} | {:error, VialKeeper.Error.t()}
  def count(conn) do
  ...
  def get_definition(conn, view_id) when is_binary(view_id) do
    case Connection.query(
      ...
      {:ok, []} ->
        {:error, VialKeeper.Error.view_not_found("view was not found", %{view_id: view_id})}
  ...
  @spec get_state(Connection.handle(), binary()) :: {:ok, map()} | {:error, VialKeeper.Error.t()}
  ...
  @spec delete(Connection.handle(), binary()) :: {:ok, map()} | {:error, VialKeeper.Error.t()}
  ...
  defp decode_body(body_json, body_term) do
    case TermBlob.decode(body_term, body_json) do
      ...
      {:fallback, reason} ->
        {:error, VialKeeper.Error.internal_error("term blob decode failed", %{reason: reason})}
    end
  end
