defmodule VialKeeper.Storage.SQLite.LocalRecords do
  @moduledoc """
  ...
  @spec fetch(Connection.handle(), binary(), binary()) ::
          {:ok, nil | map()} | {:error, VialKeeper.Error.t()}
  ...
  """
  @spec put_cas_tx(map(), map()) :: {:ok, map()} | {:error, VialKeeper.Error.t()}
  ...
  defp validate_request(namespace, key, expected, _observed, _current, _json)
       when not is_binary(namespace) or not is_binary(key) or not is_integer(expected) or
              expected < 0,
       do: {:error, VialKeeper.Error.invalid_request("local record CAS fields are invalid")}
