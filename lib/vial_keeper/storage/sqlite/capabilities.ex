defmodule VialKeeper.Storage.SQLite.Capabilities do
  @moduledoc """
  ...
  def validate! do
    with {:ok, conn} <- Connection.open(":memory:"),
      ...
      reason ->
        Application.put_env(:vial_keeper, :fts5_contentless_delete, false)
  ...
  def fts5_contentless_delete_supported? do
    case Application.get_env(:vial_keeper, :fts5_contentless_delete) do
      true ->
        true
      ...
      nil ->
        case Connection.open(":memory:") do
          {:ok, conn} ->
            ...
            Application.put_env(:vial_keeper, :fts5_contentless_delete, result)
            _ = Connection.close(conn)
            result
