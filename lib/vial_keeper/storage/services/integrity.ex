defmodule VialKeeper.Storage.Services.Integrity do
  @moduledoc """
  Shared integrity orchestration over storage inspection ports.

  Loads a normalized logical snapshot, runs `VialKeeper.Integrity.Rules`, and
  ...
  """
  @spec check(BackendContext.t(), map()) :: {:ok, map()} | {:error, VialKeeper.Error.t()}
  ...
  defp load_snapshot(inspection, context, options) do
    ...
    if function_exported?(inspection, :load_integrity_snapshot, 2) do
      ...
      {:error,
       VialKeeper.Error.internal_error("inspection port does not load integrity snapshots", %{
         port: inspection
       })}
    end
  end
