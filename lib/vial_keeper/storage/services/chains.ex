defmodule VialKeeper.Storage.Services.Chains do
  @moduledoc """
  ...
  """
  @spec diff(BackendContext.t(), map()) :: {:ok, map()} | {:error, VialKeeper.Error.t()}
  ...
  """
  @spec get(BackendContext.t(), map()) :: {:ok, map()} | {:error, VialKeeper.Error.t()}
  ...
  defp bootstrap_page_size do
    VialKeeper.Config.host_limits()[:max_bulk_operations] || 500
  end
  ...
  defp chain_for_leaf(context, doc, leaf_id, truncated) do
    case Facts.find_revision(context, doc.document_id, leaf_id) do
      ...
      {:ok, nil} ->
        {:error, VialKeeper.Error.revision_not_found("revision not found", %{revision: leaf_id})}
  ...
  defp chain(context, document_id, revision, acc, opts) do
    ...
    if truncated do
      ...
      case Facts.list_ancestors(context, document_id, revision.revision_id) do
        ...
        {:error, %VialKeeper.Error{code: :revision_not_found, details: details}} ->
          {:error,
           VialKeeper.Error.integrity_violation("revision chain contains a dangling parent", %{
  ...
  defp validate_documents_batch(documents) when is_list(documents) do
    ...
    cond do
      length(documents) > max ->
        {:error,
         VialKeeper.Error.resource_limit("replication document count exceeds the host limit")}

      not Enum.all?(documents, &valid_replication_document_request?/1) ->
        {:error, VialKeeper.Error.invalid_request("replication document request is invalid")}
  ...
  defp validate_documents_batch(_),
    do: {:error, VialKeeper.Error.invalid_request("replication documents must be an array")}
  ...
  defp valid_replication_document_request?(entry) when is_map(entry) do
    ...
    Enum.all?(Map.keys(entry), &(&1 in allowed)) and is_binary(id) and
      ...
      length(leaves) <= (VialKeeper.Config.host_limits()[:max_bulk_operations] || 500) and
  ...
  defp validate_bootstrap_limit(_),
    do: {:error, VialKeeper.Error.invalid_request("bootstrap limit must be a positive integer")}
  ...
  defp validate_bootstrap_cursor(_),
    do: {:error, VialKeeper.Error.invalid_request("bootstrap cursor must be a binary or null")}

  defp validate_truncated_flag(value) when value in [true, false], do: :ok

  defp validate_truncated_flag(_),
    do: {:error, VialKeeper.Error.invalid_request("truncated must be a boolean")}
end
