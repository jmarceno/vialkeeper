defmodule VialKeeper.Storage.Services.Attachments do
  @moduledoc """
  ...
  alias VialKeeper.Attachments.{MetadataRequest, Orchestration}
  ...
  def resolve_blob_metadata(_context, _request),
    do: {:error, VialKeeper.Error.invalid_request("blob metadata request must be an object")}
  ...
  def remove_pending_blob_protection(_context, _request),
    do:
      {:error,
       VialKeeper.Error.invalid_request("remove pending blob protection request must be an object")}
  ...
  def list_live_attachment_digests(%BackendContext{} = context, request) when is_map(request) do
    after_digest = VialKeeper.MapAccess.get(request, :after_digest)
    limit = VialKeeper.MapAccess.get(request, :limit)
    now = DateTime.utc_now() |> DateTime.truncate(:second)
  ...
  def list_live_attachment_digests(_context, _request),
    do:
      {:error, VialKeeper.Error.invalid_request("live attachment digest request must be an object")}
