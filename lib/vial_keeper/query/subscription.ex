defmodule VialKeeper.Query.Subscription do
  @moduledoc "Supervised live-query subscription process for one client."
  use GenServer

  alias VialKeeper.Query.Subscription.{Events, Membership}
  ...
  def handle_call(:next, _from, %{waiter: waiter} = state) when not is_nil(waiter) do
    {:reply, {:error, VialKeeper.Error.invalid_request("only one next caller is allowed")}, state}
  end
  ...
  defp refresh_reset_limits(state) do
    ...
    |> case do
      ...
      {:ok, _identity} ->
        {:error, VialKeeper.Error.database_unavailable("subscription configuration is invalid")}
  ...
  @spec normalize_hub_result(term()) :: {:ok, term()} | {:error, VialKeeper.Error.t()}
  ...
  defp sync_activate_barrier(uuid) when is_binary(uuid) do
    case Application.get_env(:vial_keeper, :subscription_activate_sync) do
      {pid, ref, ^uuid} when is_pid(pid) and is_reference(ref) ->
  ...
  @spec normalize_catalog_result({:ok, term()} | {:error, VialKeeper.Error.t()}) ::
          {:ok, term()} | {:error, VialKeeper.Error.t()}
  defp normalize_catalog_result({:ok, _} = ok), do: ok
  defp normalize_catalog_result({:error, %VialKeeper.Error{}} = error), do: error
