defmodule VialKeeper.Query.SubscriptionRequest do
  ...
  @spec normalize(map(), map()) :: {:ok, map()} | {:error, VialKeeper.Error.t()}
  ...
  def normalize(_, _),
    do: {:error, VialKeeper.Error.invalid_request("subscription request must be an object")}

  @doc """
  Validates and normalizes a subscription snapshot command request.
  """
  @spec prepare_snapshot(map(), map()) :: {:ok, map()} | {:error, VialKeeper.Error.t()}
  ...
  defp validate_allowed_string(string, allowed, unknown_message) do
    if MapSet.member?(allowed, string),
      do: {:cont, :ok},
      else: {:halt, {:error, VialKeeper.Error.invalid_request(unknown_message)}}
  end

  defp require_query_object(request) do
    case get(request, :query) do
      query when is_map(query) -> {:ok, query}
      nil -> {:error, VialKeeper.Error.invalid_request("subscription request requires query")}
      _ -> {:error, VialKeeper.Error.invalid_request("query must be an object")}
    end
  end
  ...
  defp reject_forbidden_query_fields(request) do
    with {:ok, keys} <- request_string_keys(request) do
      case Enum.find(@forbidden_query_keys, &MapSet.member?(keys, &1)) do
        nil -> :ok
        field -> {:error, VialKeeper.Error.invalid_request("subscription query rejects #{field}")}
      end
    end
  end
  ...
  defp stringify_key(_),
    do: {:error, VialKeeper.Error.invalid_request("request contains an invalid field name")}
  ...
  defp validate_max_members(value, config) when is_integer(value) and value > 0 do
    ...
    if value <= maximum,
      do: {:ok, value},
        ...
        {:error,
         VialKeeper.Error.resource_limit("max_members exceeds the configured maximum", %{
           maximum: maximum,
           value: value
         })}
  end
  ...
  defp validate_max_members(_value, _config),
    do: {:error, VialKeeper.Error.invalid_request("max_members must be a positive integer")}
  ...
  defp validate_revision_batch_size(requests, maximum) do
    if Enum.count(requests) > maximum do
      {:error,
       VialKeeper.Error.resource_limit("revision batch exceeds the configured maximum", %{
         maximum: maximum,
  ...
  defp validate_revision_batch_item(_),
    do: {:error, VialKeeper.Error.invalid_request("revision batch item must be an object")}
  ...
  defp validate_required_binary(request, key) do
    case get(request, key) do
      ...
      nil -> {:error, VialKeeper.Error.invalid_request("#{key} is required")}
  ...
  defp normalize_heartbeat(request, config) do
    ...
    case get(request, :heartbeat_ms) do
      ...
      value when is_integer(value) and value > maximum ->
        {:error,
         VialKeeper.Error.resource_limit("heartbeat_ms exceeds the configured maximum", %{
           maximum: maximum,
           value: value
         })}

      _ ->
        {:error, VialKeeper.Error.invalid_request("heartbeat_ms must be a positive integer")}
    end
  end
