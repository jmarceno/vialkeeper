defmodule VialKeeper.Query.SubscriptionHub do
  ...
  defp subscription_hub_reads_paused?(uuid) when is_binary(uuid) do
    case Application.get_env(:vial_keeper, :subscription_hub_pause_reads) do
      ^uuid -> true
      true -> true
      _ -> false
    end
  ...
  defp cancel_read_task(%{read_task: %Task{pid: pid} = task} = state) do
    _ = Task.Supervisor.terminate_child(VialKeeper.TaskSupervisor, pid)
