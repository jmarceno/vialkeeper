defmodule VialKeeper.Query.Planner do
  @moduledoc """
  ...
  alias VialKeeper.Query.{Plan, Predicate}
  ...
  """
  @spec plan([map()], map()) :: {:ok, Plan.t()} | {:error, VialKeeper.Error.t()}
  ...
  def plan(_indexes, _request),
    do: {:error, VialKeeper.Error.invalid_request("query planning requires an object")}
  ...
  defp plan_full_text(indexes, request) do
    ...
    case Enum.find(indexes, &full_text_index?(&1, requested)) do
      nil ->
        {:error,
         VialKeeper.Error.invalid_index_hint(
  ...
  defp plan_explicit_structured(indexes, request) do
    ...
    case Enum.find(indexes, &index_named?(&1, requested)) do
      nil ->
        {:error,
         VialKeeper.Error.invalid_index_hint("requested index does not exist", %{index: requested})}
  ...
  defp invalid_hint(index),
    do:
      {:error,
       VialKeeper.Error.invalid_index_hint("requested index is incompatible with the query", %{
         index: index
       })}
  ...
  @spec select_index([map()], map()) :: {:ok, map() | nil} | {:error, VialKeeper.Error.t()}
    ...
    case requested_index_name(request) do
      ...
      name ->
        case Enum.find(indexes, &(&1["name"] == name or &1["index_id"] == name)) do
          nil ->
            {:error,
             VialKeeper.Error.invalid_index_hint("requested index does not exist", %{index: name})}
