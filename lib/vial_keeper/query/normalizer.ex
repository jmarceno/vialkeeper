defmodule VialKeeper.Query.Normalizer do
  @moduledoc "Validates and canonicalizes the storage-neutral query request."

  alias VialKeeper.JSON.{Canonical, Pointer, Stringify}
  alias VialKeeper.MapAccess
  alias VialKeeper.Query.Predicate
  alias VialKeeper.Query.Prepared
  ...
  def normalize(_), do: {:error, VialKeeper.Error.invalid_request("query must be an object")}
  ...
  defp known_fields(request) do
    ...
    if Enum.all?(Map.keys(request), &MapSet.member?(allowed, &1)),
      do: :ok,
      else: {:error, VialKeeper.Error.invalid_request("query contains an unknown field")}
  end
      ...
      {:ok, normalized, predicate, _state} ->
        case Predicate.introspect(predicate) do
          ...
          %{depth: depth} when depth > @max_depth ->
            {:error,
             VialKeeper.Error.resource_limit("query predicate depth exceeds the configured limit")}

          _ ->
            {:error, VialKeeper.Error.resource_limit("query predicate is too complex")}
        end
  ...
  defp normalize_selector(_),
    do: {:error, VialKeeper.Error.invalid_request("selector must be an object")}
  ...
  defp compile_selector_entry("$not", value, state, depth) do
    with true <- is_map(value),
      ...
      false -> {:error, VialKeeper.Error.invalid_request("$not requires one selector object")}
      {:error, _} = error -> error
    end
  end
  ...
  defp compile_selector_entry(key, value, state, depth) when is_binary(key) do
    if String.starts_with?(key, "$") do
      {:error, VialKeeper.Error.invalid_request("unsupported selector operator")}
    else
  ...
  defp compile_selector_entry(_key, _value, _state, _depth),
    do: {:error, VialKeeper.Error.invalid_request("selector field must be a string")}

  defp compile_boolean(operator, value, state, depth)
       when operator in ["$and", "$or", "$nor"] do
    cond do
      not is_list(value) or value == [] ->
        {:error, VialKeeper.Error.invalid_request("#{operator} must be a non-empty array")}

      length(value) > @max_boolean_children ->
        {:error, VialKeeper.Error.resource_limit("#{operator} has too many child selectors")}
  ...
  defp compile_field_entry(path, value, state, depth) do
    with {:ok, [_ | _]} <- Pointer.parse(path),
      ...
      {:ok, []} ->
        {:error, VialKeeper.Error.invalid_request("selector paths must not be empty")}
  ...
  defp compile_operator_entry(key, _value, _predicates, normalized, _state, _depth)
    ...
    {:halt,
     {:error, VialKeeper.Error.invalid_request("selector operator keys must be unique strings")}}
  end
  ...
  defp compile_operator("$exists", _value, _state, _depth),
    do: {:error, VialKeeper.Error.invalid_request("$exists requires a boolean")}
  ...
  defp compile_operator("$type", _value, _state, _depth),
    do: {:error, VialKeeper.Error.invalid_request("$type requires a supported JSON type")}
  ...
  defp compile_operator("$beginsWith", _value, _state, _depth),
    do: {:error, VialKeeper.Error.invalid_request("$beginsWith requires a non-empty UTF-8 string")}
  ...
  defp compile_operator("$regex", _value, _state, _depth),
    do: {:error, VialKeeper.Error.invalid_request("$regex requires a string")}
  ...
  defp compile_operator("$elemMatch", _value, _state, _depth),
    do: {:error, VialKeeper.Error.invalid_request("$elemMatch requires a selector object")}
  ...
  defp compile_operator("$size", _value, _state, _depth),
    do: {:error, VialKeeper.Error.invalid_request("$size requires a non-negative integer")}
  ...
  defp compile_operator("$mod", [divisor, remainder], state, depth) do
    with {:ok, [divisor, remainder]} <- normalize_mod_values([divisor, remainder]),
      ...
      true -> {:error, VialKeeper.Error.invalid_request("$mod divisor must not be zero")}
      {:error, _} = error -> error
    end
  end

  defp compile_operator("$mod", _value, _state, _depth),
    do: {:error, VialKeeper.Error.invalid_request("$mod requires two safe integers")}

  defp compile_operator(_operator, _value, _state, _depth),
    do: {:error, VialKeeper.Error.invalid_request("unsupported selector operator")}
  ...
  defp ordered_value(_value, operator),
    do: {:error, VialKeeper.Error.invalid_request("#{operator} requires a number or string")}

  defp validate_scalar_list(operator, values) when is_list(values) and values != [] do
    max = VialKeeper.Config.host_limits()[:max_query_results] || 500

    cond do
      length(values) > max ->
        {:error, VialKeeper.Error.resource_limit("#{operator} values exceed the configured limit")}

      Enum.all?(values, &Predicate.scalar?/1) ->
        :ok

      true ->
        {:error, VialKeeper.Error.invalid_request("#{operator} requires non-empty scalar values")}
    end
  end

  defp validate_scalar_list(operator, _values),
    do: {:error, VialKeeper.Error.invalid_request("#{operator} requires a non-empty array")}

  defp normalize_mod_values(values) do
    if Enum.all?(values, &safe_integer?/1),
      do: {:ok, values},
      else: {:error, VialKeeper.Error.invalid_request("$mod requires two safe integers")}
  end
  ...
  defp validate_non_empty_utf8(value, label) do
    if value != "" and String.valid?(value),
      do: :ok,
      else: {:error, VialKeeper.Error.invalid_request("#{label} requires a non-empty UTF-8 string")}
  end
  ...
  defp normalize_json(value) do
    ...
    if valid_json?(normalized),
      do: {:ok, normalized},
      else: {:error, VialKeeper.Error.invalid_request("selector values must be valid JSON")}
  rescue
    Protocol.UndefinedError ->
      {:error, VialKeeper.Error.invalid_request("selector values must be valid JSON")}
  end
  ...
  defp normalize_sort(values) when is_list(values) do
    Enum.reduce_while(values, {:ok, []}, fn value, {:ok, acc} ->
      ...
      with true <- is_map(value),
        ...
        false ->
          {:halt, {:error, VialKeeper.Error.invalid_request("sort direction must be asc or desc")}}

        _ ->
          {:halt,
           {:error, VialKeeper.Error.invalid_request("sort fields require non-empty JSON Pointers")}}
      end
    end)
    |> then(fn
  ...
  defp normalize_sort(_), do: {:error, VialKeeper.Error.invalid_request("sort must be an array")}

  defp normalize_fields(nil), do: {:ok, nil}

  defp normalize_fields(values) when is_list(values) do
    if values == [] do
      {:error, VialKeeper.Error.invalid_request("fields must be a non-empty array")}
    else
      normalize_field_list(values)
    end
  end

  defp normalize_fields(_),
  ...
  defp normalize_field_list(values) do
    Enum.reduce_while(values, {:ok, []}, fn path, {:ok, acc} ->
      with true <- is_binary(path),
        ...
        _ ->
          {:halt,
           {:error, VialKeeper.Error.invalid_request("fields require non-empty JSON Pointers")}}
      end
    end)
    |> then(fn
      {:ok, values} -> {:ok, Enum.reverse(values)}
  ...
  defp normalize_index_name(_),
    do: {:error, VialKeeper.Error.invalid_request("index must be a string")}
  ...
  defp normalize_search(value) when is_map(value) do
    ...
    with true <- Map.keys(value) -- [:index, :text, :mode, "index", "text", "mode"] == [],
      ...
      _ ->
        {:error,
         VialKeeper.Error.invalid_request("search requires index, text, and a supported mode")}
    end
  end

  defp normalize_search(_),
    do: {:error, VialKeeper.Error.invalid_request("search must be an object")}
    ...
    Enum.reduce_while(value, {:ok, MapSet.new()}, fn {key, child}, {:ok, seen} ->
      with {:ok, string_key} <- key_as_string(key),
        ...
        true ->
          {:halt,
           {:error,
            VialKeeper.Error.invalid_request("object keys must be unique after stringification")}}
  ...
  defp key_as_string(_key), do: {:error, VialKeeper.Error.invalid_request("object key is invalid")}
end
