defmodule VialKeeper.Query.Plan do
  @moduledoc """
  ...
  def new(_), do: {:error, VialKeeper.Error.invalid_request("query plan must be an object")}
  ...
  def transitional_digest(_kind, _binding),
    do: {:error, VialKeeper.Error.invalid_request("query plan transitional binding is invalid")}

  defp validate_kind(kind) when kind in @kinds, do: :ok

  defp validate_kind(_),
    do: {:error, VialKeeper.Error.invalid_request("query plan kind is invalid")}

  defp validate_sort_compatible(value) when is_boolean(value), do: :ok

  defp validate_sort_compatible(_),
    do: {:error, VialKeeper.Error.invalid_request("query plan sort_compatible? must be boolean")}
  ...
  defp invalid_pagination,
    do: {:error, VialKeeper.Error.invalid_request("query plan pagination is invalid")}
  ...
  defp normalize_bindings(_),
    do: {:error, VialKeeper.Error.invalid_request("query plan selected_indexes must be an array")}

  defp append_binding(normalized, acc, ids) do
    if MapSet.member?(ids, normalized.index_id) do
      {:halt,
       {:error, VialKeeper.Error.invalid_request("query plan index bindings must be unique")}}
    else
  ...
  defp normalize_binding(binding) when is_map(binding) do
    with :ok <- validate_no_key_collisions(binding),
      ...
      {:error, _} = error -> error
      _ -> {:error, VialKeeper.Error.invalid_request("query plan index binding is invalid")}
    end
  end

  defp normalize_binding(_),
    do: {:error, VialKeeper.Error.invalid_request("query plan index binding must be an object")}
  ...
  defp sanitize_scans(scans) when is_list(scans) do
    Enum.reduce_while(scans, {:ok, []}, fn scan, {:ok, acc} ->
      case sanitize_value(scan) do
        ...
        {:ok, _} ->
          {:halt, {:error, VialKeeper.Error.invalid_request("query plan scans must be objects")}}
  ...
  defp sanitize_scans(_),
    do: {:error, VialKeeper.Error.invalid_request("query plan scans must be an array")}
  ...
  defp sanitize_value(_value),
    do: {:error, VialKeeper.Error.invalid_request("query plan contains an invalid value")}
  ...
  defp sanitize_map_key(string_key, _child, acc, keys) when string_key in @runtime_keys do
    if MapSet.member?(keys, string_key) do
      {:halt, {:error, VialKeeper.Error.invalid_request("query plan keys must be unique")}}
    else
  ...
  defp stringify_key(_key),
    do: {:error, VialKeeper.Error.invalid_request("query plan key is invalid")}
  ...
  defp invalid_shape(message), do: {:error, VialKeeper.Error.invalid_request(message)}
end
