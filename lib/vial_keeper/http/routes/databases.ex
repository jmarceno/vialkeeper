defmodule VialKeeper.HTTP.Routes.Databases do
  ...
  post "/" do
    Request.call(
      ...
      fn body, conn ->
        path = body["path"] || "#{VialKeeper.UUID.v4()}.vialkeeper"

        if Map.has_key?(body, "path") and not is_binary(body["path"]) do
          Response.error(conn, VialKeeper.Error.invalid_request("database path must be a string"))
        else
          case body["config"] do
            ...
            _ ->
              Response.error(
                conn,
                VialKeeper.Error.invalid_request("database config must be an object")
              )
          end
        end
      end
    ...
    Request.call(conn, fn body, conn ->
      if is_map(body),
        do:
          ...
          Response.error(conn, VialKeeper.Error.invalid_request("configuration must be an object"))
    end)
  end
  ...
  post "/:uuid/integrity-check" do
    case Maintenance.integrity_check(Request.uuid(conn)) do
      {:ok, data} -> Response.ok(conn, data)
      {:error, error} -> Response.error(conn, error)
    end
  end
  ...
  post "/:uuid/compact" do
    Request.call(
      ...
      fn body, conn ->
        Response.result(
          conn,
          Maintenance.compact(Request.uuid(conn), body || %{})
        )
      end
    )
  end
