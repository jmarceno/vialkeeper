defmodule VialKeeper.HTTP.Routes.Documents do
        ...
        case Request.read_consistency(conn) do
          {:ok, consistency} ->
            Response.result_with_read_meta(
              conn,
              VialKeeper.Documents.get_with_meta(Request.uuid(conn), body,
  ...
  post "/put" do
    Request.call(
      ...
      fn body, conn ->
        Response.result(conn, VialKeeper.Documents.put(Request.uuid(conn), body), 201)
      end
    )
  end
  ...
  post "/delete" do
    Request.call(
      ...
      fn body, conn ->
        Response.result(conn, VialKeeper.Documents.delete(Request.uuid(conn), body))
      end
    )
  end
  ...
  post "/resolve" do
    Request.call(
      ...
      fn body, conn ->
        Response.result(conn, VialKeeper.Documents.resolve(Request.uuid(conn), body))
      end
    )
  end
    ...
    Request.call(conn, fn body, conn ->
      case Request.read_consistency(conn) do
        {:ok, consistency} ->
          Response.result_with_read_meta(
            conn,
            VialKeeper.Documents.bulk_get_with_meta(
              Request.uuid(conn),
              body,
              read_consistency: consistency
            )
  ...
  post "/bulk-write" do
    Request.call(conn, fn body, conn ->
      Response.result(conn, VialKeeper.Documents.bulk_write(Request.uuid(conn), body))
    end)
  end
