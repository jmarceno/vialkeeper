defmodule VialKeeper.HTTP.Routes.Shadows do
    ...
    with :ok <- source_uuid_valid(source_uuid),
      ...
      cond do
        enabled and not controller_enabled?() ->
          Response.error(conn, VialKeeper.Error.invalid_request("shadow controller is disabled"))
  ...
  defp update_shadow(conn, _body),
    do:
      Response.error(
        conn,
        VialKeeper.Error.invalid_request("shadow configuration must be an object")
      )
  ...
  defp enqueue(definition) do
    case Reconciler.enqueue(definition) do
      ...
      {:error, reason} ->
        {:error,
         VialKeeper.Error.database_unavailable("shadow reconciliation could not be scheduled", %{
      ...
      {:ok, entries} ->
        if Enum.any?(entries, &(&1.database_uuid == source_uuid and &1.database_kind == :ordinary)),
          do: :ok,
            ...
            {:error,
             VialKeeper.Error.database_not_found(
  ...
  defp source_uuid_valid(value) when is_binary(value) do
    if Regex.match?(@uuid_pattern, value),
      do: :ok,
      else: {:error, VialKeeper.Error.database_not_found("source database is not registered")}
  end

  defp source_uuid_valid(_),
    do: {:error, VialKeeper.Error.database_not_found("source database is not registered")}
  ...
  defp enabled_value(_),
    do: {:error, VialKeeper.Error.invalid_request("shadow enabled must be a boolean")}
