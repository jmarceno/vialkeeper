defmodule VialKeeper.HTTP.Routes.Changes do
  ...
  alias VialKeeper.Runtime.ChangeNotifier
  plug(:match)
  plug(:dispatch)
  ...
  post "/" do
    Request.call(
      ...
      fn body, conn ->
        Response.result(conn, VialKeeper.Changes.wait(Request.uuid(conn), body))
      end
    )
  end
    ...
    Request.call(
      ...
      fn body, conn ->
        with :ok <- validate_stream_request(body),
          ...
          {:error, %VialKeeper.Error{} = error} -> Response.error(conn, error)
          {:error, _reason} -> conn
        end
      end
    )
  ...
  defp validate_stream_request(_),
    do: {:error, VialKeeper.Error.invalid_request("changes stream request must be an object")}
  ...
  defp valid_stream_limit_max(_value, _max),
    do: VialKeeper.Error.resource_limit("changes stream limit exceeds the host limit")

  defp valid_heartbeat(value) when is_integer(value) and value >= 0, do: nil

  defp valid_heartbeat(_),
    do: VialKeeper.Error.invalid_request("heartbeat_ms must be a non-negative integer")

  defp valid_heartbeat_max(value, max) when value <= max, do: nil

  defp valid_heartbeat_max(_value, _max),
    do: VialKeeper.Error.resource_limit("heartbeat_ms exceeds the host limit")
  ...
  defp stream_read(uuid, body) do
    ...
    VialKeeper.Changes.read(uuid, request)
  end
  ...
  defp stream_error(conn, %VialKeeper.Error{} = error) do
    _ =
      Plug.Conn.chunk(conn, [
        JSON.encode_to_iodata!(%{"type" => "error", "error" => VialKeeper.Error.public(error)}),
        "\n"
      ])
