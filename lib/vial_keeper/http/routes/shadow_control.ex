defmodule VialKeeper.HTTP.Routes.ShadowControl do
  ...
  alias VialKeeper.Shadow.{Protocol, Worker}
      ...
      Request.call(
        ...
        fn body, conn ->
          ...
          if body["source_uuid"] == source_uuid and body["generation"] == generation do
            ...
            Response.error(
              conn,
              VialKeeper.Error.shadow_generation_conflict("shadow path and request identity differ")
            )
          end
        end
      )
    ...
    read_request(conn, @read_fields, fn conn, body ->
      case Worker.open_attachment_representation(body, [], []) do
        ...
        when is_integer(watermark) and watermark >= 0 ->
          ...
          |> Plug.Conn.put_resp_header("x-vialkeeper-attachment-content-type", content_type)
          |> send_representation(stream)

        {:ok, _} ->
          Response.error(
            conn,
            VialKeeper.Error.shadow_incompatible("shadow attachment response is invalid")
          )
  ...
  defp parse_generation(value) when is_binary(value) do
    case Integer.parse(value) do
      ...
      _ ->
        {:error, VialKeeper.Error.invalid_request("shadow generation must be positive")}
    end
  end

  defp parse_generation(_),
    do: {:error, VialKeeper.Error.invalid_request("shadow generation is missing")}
