defmodule VialKeeper.Revisions.Tree do
  ...
  @spec add([Revision.t()], Revision.t()) :: {:ok, [Revision.t()]} | {:error, VialKeeper.Error.t()}
  def add(revisions, %Revision{} = revision) do
    case Enum.find(revisions, &(&1.revision_id == revision.revision_id)) do
      ...
      _ -> {:error, VialKeeper.Error.integrity_violation("revision id has different content")}
    end
  end

  @spec projection([Revision.t()]) :: {:ok, map()} | {:error, VialKeeper.Error.t()}
  def projection(revisions) do
    with leaves when leaves != [] <- leaves(revisions), {:ok, winner} <- Winner.select(leaves) do
      ...
      _ -> {:error, VialKeeper.Error.document_not_found("revision tree is empty")}
    end
  end
end
