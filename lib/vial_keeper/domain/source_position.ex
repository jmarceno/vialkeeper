defmodule VialKeeper.Domain.SourcePosition do
  ...
  def new(attrs) when is_map(attrs) do
    if Enum.any?(Map.keys(attrs), &(&1 not in @known)),
      do: {:error, VialKeeper.Error.invalid_request("unknown source position field")},
      else: build(attrs)
  end

  def new(_), do: {:error, VialKeeper.Error.invalid_request("source position must be an object")}
  ...
  def from_wire(attrs) when is_map(attrs) do
    ...
    if Enum.any?(Map.keys(attrs), &(&1 not in allowed)) do
      {:error, VialKeeper.Error.invalid_request("unknown source position field")}
    else
      new(%{
  ...
  def from_wire(_),
    do: {:error, VialKeeper.Error.invalid_request("source position must be an object")}
  ...
  defp validate_database_uuid(_),
    do: VialKeeper.Error.invalid_request("source position database_uuid is required")
  ...
  defp validate_history_epoch(_),
    do: VialKeeper.Error.invalid_request("source position history_epoch is required")

  defp validate_sequence(%{sequence: value}) when is_integer(value) and value >= 0, do: nil

  defp validate_sequence(_),
    do: VialKeeper.Error.invalid_request("source position sequence must be non-negative")
end
