defmodule VialKeeper.Domain.Change do
  ...
  def new(attrs) when is_map(attrs) do
    if Enum.any?(Map.keys(attrs), &(&1 not in @known)),
      do: {:error, VialKeeper.Error.invalid_request("unknown change field")},
      else: build(attrs)
  end

  def new(_), do: {:error, VialKeeper.Error.invalid_request("change must be an object")}
  ...
  defp normalize_leaves(_),
    do: {:error, VialKeeper.Error.invalid_request("leaf_revisions must be an array")}
  ...
  defp normalize_leaf(_),
    do: {:error, VialKeeper.Error.invalid_request("leaf_revisions entries must be objects")}
  ...
  defp require_binary(_, label),
    do: {:error, VialKeeper.Error.invalid_request("#{label} is required")}

  defp require_boolean(value, _label) when is_boolean(value), do: :ok

  defp require_boolean(_, label),
    do: {:error, VialKeeper.Error.invalid_request("#{label} must be boolean")}
  ...
  defp optional_binary(_, label),
    do: {:error, VialKeeper.Error.invalid_request("#{label} must be a string")}
end
