defmodule VialKeeper.Domain.BoundaryPage do
  @moduledoc "Validated compact-retention boundary page for wire transfer."

  alias VialKeeper.Domain.RetentionBoundary
  ...
  def new(attrs) when is_map(attrs) do
    if Enum.any?(Map.keys(attrs), &(&1 not in @known)),
      do: {:error, VialKeeper.Error.invalid_request("unknown boundary page field")},
      else: build(attrs)
  end

  def new(_), do: {:error, VialKeeper.Error.invalid_request("boundary page must be an object")}
  ...
  def from_wire(attrs) when is_map(attrs) do
    ...
    if Enum.any?(Map.keys(attrs), &(&1 not in allowed)) do
      {:error, VialKeeper.Error.invalid_request("unknown boundary page field")}
    else
  ...
  defp normalize_boundaries(_),
    do: {:error, VialKeeper.Error.invalid_request("boundary page boundaries must be an array")}
  ...
  defp normalize_boundary(_),
    do:
      {:error, VialKeeper.Error.invalid_request("boundary page boundaries entries must be objects")}
  ...
  defp validate_source_history_epoch(_),
    do: {:error, VialKeeper.Error.invalid_request("boundary page source_history_epoch is required")}
  ...
  defp validate_boundary_digest(_),
    do: {:error, VialKeeper.Error.invalid_request("boundary page boundary_digest is required")}
  ...
  defp validate_next_page(_),
    do:
      {:error, VialKeeper.Error.invalid_request("boundary page next_page must be a binary or null")}
  ...
  defp validate_install_id(_),
    do:
      {:error,
       VialKeeper.Error.invalid_request("boundary page install_id must be a binary or null")}

  defp validate_replace(value) when is_boolean(value), do: nil

  defp validate_replace(_),
    do: {:error, VialKeeper.Error.invalid_request("boundary page replace must be boolean")}
