defmodule VialKeeper.Domain.PeerPosition do
  @moduledoc "Validated peer-ledger position for stable-frontier retention."

  alias VialKeeper.Domain.ValidatedStruct
  ...
  def new(attrs) when is_map(attrs) do
    if Enum.any?(Map.keys(attrs), &(&1 not in @known)),
      do: {:error, VialKeeper.Error.invalid_request("unknown peer position field")},
      else: ValidatedStruct.build(__MODULE__, attrs, &peer_position_validation_error/1)
  end

  def new(_), do: {:error, VialKeeper.Error.invalid_request("peer position must be an object")}
  ...
  def from_wire(attrs) when is_map(attrs) do
    ...
    if Enum.any?(Map.keys(attrs), &(&1 not in allowed)) do
      {:error, VialKeeper.Error.invalid_request("unknown peer position field")}
    else
  ...
  defp validate_peer_database_uuid(_),
    do: VialKeeper.Error.invalid_request("peer position peer_database_uuid is required")
  ...
  defp validate_peer_history_epoch(_),
    do: VialKeeper.Error.invalid_request("peer position peer_history_epoch is required")
  ...
  defp validate_source_database_uuid(_),
    do: VialKeeper.Error.invalid_request("peer position source_database_uuid is required")
  ...
  defp validate_source_history_epoch(_),
    do: VialKeeper.Error.invalid_request("peer position source_history_epoch is required")
  ...
  defp validate_safe_source_sequence(_),
    do: VialKeeper.Error.invalid_request("peer position safe_source_sequence must be non-negative")
  ...
  defp validate_last_seen_at(_),
    do: VialKeeper.Error.invalid_request("peer position last_seen_at is required")
  ...
  defp validate_lease_expires_at(_),
    do: VialKeeper.Error.invalid_request("peer position lease_expires_at is required")
