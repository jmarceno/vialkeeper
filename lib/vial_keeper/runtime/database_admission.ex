defmodule VialKeeper.Runtime.DatabaseAdmission do
  ...
  def via(uuid), do: {:via, Registry, {VialKeeper.Runtime.DatabaseRegistry, {:admission, uuid}}}
  ...
  defp sync_owner_for_drain(uuid) do
    timeout = Application.get_env(:vial_keeper, :executor_drain_sync_timeout, :infinity)
  ...
  defp drain_sync_retry_ms do
    Application.get_env(:vial_keeper, :executor_drain_sync_retry_ms, 10)
  end
  ...
  defp probe_admission_class(class, probe_op) do
    case Application.get_env(:vial_keeper, :admission_class_probe) do
  ...
  defp notify_test_hook(:enqueued, request_ref, class, probe_op, caller_pid) do
    case Application.get_env(:vial_keeper, :admission_test_hook) do
      {pid, ref} when is_pid(pid) and is_reference(ref) ->
