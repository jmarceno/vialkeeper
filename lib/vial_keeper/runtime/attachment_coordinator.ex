defmodule VialKeeper.Runtime.AttachmentCoordinator do
  ...
  defp call(uuid, message, timeout \\ 30_000) do
    case Registry.lookup(VialKeeper.Runtime.DatabaseRegistry, {:attachment_coordinator, uuid}) do
      [{pid, _}] ->
        try do
          GenServer.call(pid, message, timeout)
        catch
          :exit, _ ->
            {:error, VialKeeper.Error.database_closed("attachment coordinator is not running")}
        end
  ...
  def handle_call({:end_gc, _gc_token}, _from, state) do
    {:reply, {:error, VialKeeper.Error.invalid_request("invalid attachment gc token")}, state}
  end
  ...
  defp handle_gc_caller_down(state) do
    if state.gc_waiter do
      GenServer.reply(
        state.gc_waiter,
        {:error, VialKeeper.Error.database_closed("attachment gc caller terminated")}
      )
    end
  ...
  defp abort_gc_for_close(state) do
    ...
    state =
      if state.gc_waiter do
        GenServer.reply(
          state.gc_waiter,
          {:error, VialKeeper.Error.database_closed("attachment coordinator is closing")}
        )
  ...
  defp load_limits(_uuid, :read_only) do
    defaults = VialKeeper.Config.defaults()["attachments"]
  ...
  defp ensure_open(%{closing: true}),
    do: {:error, VialKeeper.Error.database_closed("attachment coordinator is closing")}
  ...
  defp ensure_mutation_allowed(%{read_only: true}),
    do:
      {:error,
       VialKeeper.Error.shadow_command_forbidden("shadow attachment coordinator is read-only")}
  ...
  defp ensure_caller(_),
    do: {:error, VialKeeper.Error.invalid_request("attachment guard caller must be a process")}

  defp ensure_no_gc_barrier(%{gc_barrier: true}),
    do: {:error, VialKeeper.Error.attachment_overloaded("attachment gc barrier is active")}
  ...
  defp ensure_read_capacity(_),
    do: {:error, VialKeeper.Error.attachment_overloaded("attachment read limit reached")}

  defp ensure_write_capacity(%{active_writes: active, write_limit: limit}) when active < limit,
    do: :ok

  defp ensure_write_capacity(_),
    do: {:error, VialKeeper.Error.attachment_overloaded("attachment write limit reached")}
  ...
  defp release_guard(state, token) do
    case Map.pop(state.guards, token) do
      {nil, _} ->
        {:error, VialKeeper.Error.invalid_request("unknown attachment guard token")}
  ...
  defp release_guard_for_caller(state, token, caller_pid) do
    case Map.get(state.guards, token) do
      ...
      %{pid: _other} ->
        {:error, VialKeeper.Error.invalid_request("attachment guard belongs to another process")}
