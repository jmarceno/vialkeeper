defmodule VialKeeper.Replication.TransferPipeline do
  @moduledoc """
  ...
  def cancel(phase_pid) when is_pid(phase_pid) do
    send(phase_pid, :vial_keeper_replication_transfer_cancel)
    :ok
  end
  ...
  defp receive_cancel(state) do
    receive do
      :vial_keeper_replication_transfer_cancel -> %{state | cancel_requested: true}
    after
      0 -> state
    end
  end
  ...
  defp receive_one_task_result(state) do
    receive do
      :vial_keeper_replication_transfer_cancel ->
        fail_tasks(state, Error.database_closed("replication transfer was cancelled"))
  ...
  defp run_caught_transfer_task(message, fun) do
    ...
    :exit, {:vial_keeper_transfer_stream_error, %Error{} = error} -> {:error, error}
