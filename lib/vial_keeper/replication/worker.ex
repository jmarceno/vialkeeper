defmodule VialKeeper.Replication.Worker do
  @moduledoc """
  ...
  def init(options) do
    ...
    with true <- is_binary(replication_id),
         {:ok, _} <- Registry.register(VialKeeper.Replication.WorkerRegistry, replication_id, %{}) do
      data = %{
      ...
      false ->
        {:stop, VialKeeper.Error.invalid_request("replication id is required")}
    ...
    if data.cancel_requested do
      ...
      error =
        if reason in [:normal, :shutdown] do
          VialKeeper.Error.database_closed("replication task stopped")
        else
          VialKeeper.Error.internal_error("replication task crashed")
        end
  ...
  defp handle_phase_result(_state, other, data) do
    handle_failure(
      data,
      VialKeeper.Error.internal_error("unexpected replication phase result", %{
        cause: inspect(other)
      })
    )
  end
  ...
  defp cancelled_error,
    do: VialKeeper.Error.database_closed("replication was cancelled")
  ...
  defp start_batch_span(data) do
    ...
    span_ctx =
      OpenTelemetry.Tracer.start_span("vial_keeper.replication.batch", %{
        kind: :internal,
  ...
  defp end_batch_span(data, replication_id, revisions, outcome) do
    ...
    case outcome do
      :ok ->
        Meters.record(:"vial_keeper.replication.batch.duration", duration,
  ...
  defp normalize_options(options) when is_map(options) do
    session_id = MapAccess.get(options, :session_id) || VialKeeper.UUID.v4()
    Map.put(options, :session_id, session_id)
  end

  defp normalize_options(options) when is_list(options) do
    session_id = Keyword.get(options, :session_id, VialKeeper.UUID.v4())
