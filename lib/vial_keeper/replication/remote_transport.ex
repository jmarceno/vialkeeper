defmodule VialKeeper.Replication.RemoteTransport do
  ...
  def request(base_url, method, path, body \\ nil, auth_token \\ nil) do
    request(
      ...
      body,
      auth_token,
      VialKeeper.Config.host_limits()[:max_request_timeout_ms] || 30_000
    )
  end
  ...
  @spec open_stream(binary(), binary(), binary(), binary() | nil) ::
          {:ok, map(), Enumerable.t()} | {:error, VialKeeper.Error.t()}
  ...
  @doc """
  PUTs a blob representation as `application/vnd.vialkeeper.blob-representation`.
  ...
  defp stream_base_options(base_url, method, path) do
    timeout = VialKeeper.Config.host_limits()[:max_request_timeout_ms] || 30_000
