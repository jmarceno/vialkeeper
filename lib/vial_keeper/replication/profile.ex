defmodule VialKeeper.Replication.Profile do
  ...
  def validate(_),
    do: {:error, VialKeeper.Error.invalid_request("replication profile is invalid")}
  ...
  defp uuid(value, field) when is_binary(value) do
    if Regex.match?(
       ...
       do: :ok,
       else: {:error, VialKeeper.Error.invalid_request("shadow #{field} must be a UUID")}
  end
  ...
  defp positive_generation(_value),
    do: {:error, VialKeeper.Error.invalid_request("shadow generation must be positive")}
  ...
  defp different_uuids(_source, _target),
    do:
      {:error,
       VialKeeper.Error.shadow_identity_conflict("shadow source and target UUIDs must differ")}
end
