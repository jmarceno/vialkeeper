defmodule VialKeeper.Replication.LocalEndpoint do
  ...
  defp timeout, do: VialKeeper.Config.request_timeout_ms()
  ...
  def read_changes(%__MODULE__{database_uuid: uuid} = endpoint, request),
    do:
      if(MapAccess.get(request, :wait_ms, 0) > 0,
        do: VialKeeper.Changes.wait(uuid, request, admission_class: :replication),
