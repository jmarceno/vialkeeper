defmodule VialKeeper.Observability.Tracer do
  @moduledoc """
  ...
  attributes pass through `VialKeeper.Observability.Attributes.build/1` so the
  ...
  """
  @spec apply_error_status(VialKeeper.Error.t() | nil) :: VialKeeper.Error.t() | nil
  def apply_error_status(nil), do: nil

  def apply_error_status(%VialKeeper.Error{code: :internal_error} = error) do
  ...
  def apply_error_status(%VialKeeper.Error{} = error), do: error

  @doc "Records the stable error code atom on the current span (never the message)."
  @spec record_error(VialKeeper.Error.t() | nil) :: :ok
  def record_error(nil), do: :ok

  def record_error(%VialKeeper.Error{code: code}) do
