defmodule VialKeeper.Observability.Instrumentation.HTTP do
  @moduledoc """
  Emitter for the HTTP request event:

    * `vial_keeper.http.request` — span + histogram

  `VialKeeper.HTTP.Router` overrides `call/2` to wrap the whole routing pipeline
  ...
  @doc """
  Runs `fun.(conn)` (the routing pipeline) inside an `vial_keeper.http.request`
    ...
    if conn.state in [:unset, :set, :set_chunked, :set_file] do
      ...
      |> Plug.Conn.send_resp(
        ...
        JSON.encode_to_iodata!(%{
          "request_id" => VialKeeper.UUID.v4(),
          "error" =>
            VialKeeper.Error.public(
              VialKeeper.Error.internal_error("request failed", %{
                cause: inspect(error)
              })
            )
        })
