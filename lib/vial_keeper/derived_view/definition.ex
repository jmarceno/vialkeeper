defmodule VialKeeper.DerivedView.Definition do
  ...
  alias VialKeeper.View.Definition, as: ViewDefinition
  ...
  @spec normalize(map(), keyword()) :: {:ok, t()} | {:error, VialKeeper.Error.t()}
  ...
  def normalize(_definition, _opts),
    do: {:error, VialKeeper.Error.invalid_request("materialized view definition must be an object")}
  ...
  @spec validate_runtime_limits(t()) :: :ok | {:error, VialKeeper.Error.t()}
  def validate_runtime_limits(%{sources: sources}) when is_list(sources) do
    validate_source_count(
      sources,
      VialKeeper.Config.host_limits()[:max_materialized_view_sources] || 32
    )
  end

  def validate_runtime_limits(_definition),
    do: {:error, VialKeeper.Error.integrity_violation("materialized view definition is invalid")}
  ...
  defp known_fields(definition) do
    if Enum.all?(Map.keys(definition), &(&1 in @known_fields)),
      do: :ok,
        ...
        {:error,
         VialKeeper.Error.invalid_request("materialized view definition contains an unknown field")}
  end

  defp validate_plain_data(value) when is_struct(value),
    do:
      {:error,
       VialKeeper.Error.invalid_request("materialized view definition must contain JSON data")}
  ...
  defp validate_version(_),
    do: {:error, VialKeeper.Error.invalid_request("materialized view version is unsupported")}

  defp normalize_name(name) when is_binary(name) and name != "" do
    if String.valid?(name) and byte_size(name) <= 128,
      do: {:ok, name},
      else: {:error, VialKeeper.Error.invalid_request("materialized view name is invalid")}
  end

  defp normalize_name(_),
    do: {:error, VialKeeper.Error.invalid_request("materialized view name is required")}
    ...
    max_sources =
      Keyword.get(
        ...
        if(enforce_host_limits,
          do: VialKeeper.Config.host_limits()[:max_materialized_view_sources] || 32,
          else: length(sources)
        )
      )
  ...
  defp normalize_sources(_, _opts),
    do:
      {:error,
       VialKeeper.Error.invalid_request("materialized view sources must be a non-empty array")}
  ...
  defp normalize_map_definition(map_definition, name) when is_map(map_definition) do
    ...
    if Enum.all?(Map.keys(map_definition), &(&1 in ~w(selector key value))) do
      ...
      {:error, VialKeeper.Error.invalid_request("materialized view map contains an unknown field")}
    end
  end

  defp normalize_map_definition(_, _),
    do: {:error, VialKeeper.Error.invalid_request("materialized view map must be an object")}
  ...
  defp normalize_group_level(definition, nil, _key_length) do
    case Map.get(definition, "group_level") do
      nil -> {:ok, nil}
      _ -> {:error, VialKeeper.Error.invalid_request("group_level is only valid with a reducer")}
    end
  end
  ...
  defp normalize_group_level(definition, _reducer, key_length) do
    case Map.fetch(definition, "group_level") do
      ...
      {:ok, _} ->
        {:error, VialKeeper.Error.invalid_request("group_level must be within the map key length")}

      :error ->
        {:error, VialKeeper.Error.invalid_request("group_level is required with a reducer")}
    end
  end
  ...
  defp normalize_options(options, source_count, opts) when is_map(options) do
    ...
    if Enum.all?(Map.keys(options), &(&1 in @option_fields)) do
      ...
      {:error,
       VialKeeper.Error.invalid_request("materialized view options contain an unknown field")}
    end
  end
  ...
  defp validate_source_values(sources) do
    if Enum.any?(sources, &(not is_binary(&1) or not uuid?(&1))) do
      {:error,
       VialKeeper.Error.invalid_request("materialized view sources must contain UUID strings")}
    else
      :ok
    end
  end
  ...
  defp validate_source_duplicates(sources) do
    if length(Enum.uniq_by(sources, &String.downcase/1)) == length(sources) do
      ...
      {:error,
       VialKeeper.Error.invalid_request("materialized view sources must not contain duplicates")}
    end
  end
    ...
    case Keyword.get(opts, :derived_uuid) do
      ...
      derived_uuid when is_binary(derived_uuid) ->
        if String.downcase(derived_uuid) in sources do
          {:error, VialKeeper.Error.invalid_request("materialized view cannot depend on itself")}
        else
          :ok
        end
  ...
  defp validate_retry_order(options) do
    if options["retry_base_delay_ms"] <= options["retry_max_delay_ms"] do
      ...
      {:error,
       VialKeeper.Error.invalid_request("retry_base_delay_ms must be at most retry_max_delay_ms")}
    end
  end
  ...
  defp normalize_enabled(_),
    do: {:error, VialKeeper.Error.invalid_request("materialized view enabled must be boolean")}

  defp positive_option(value, _field) when is_integer(value) and value > 0, do: :ok

  defp positive_option(_value, field),
    do: {:error, VialKeeper.Error.invalid_request("materialized view #{field} must be positive")}

  defp at_most(value, maximum, _field) when value <= maximum, do: :ok

  defp at_most(_value, _maximum, field),
    do:
      {:error, VialKeeper.Error.resource_limit("materialized view #{field} exceeds the host limit")}
