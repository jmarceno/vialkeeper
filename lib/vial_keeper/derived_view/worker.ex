defmodule VialKeeper.DerivedView.Worker do
  ...
  alias VialKeeper.DerivedView.Supervisor
  alias VialKeeper.Error
  alias VialKeeper.Observability.Instrumentation.DerivedView, as: DerivedViewInstrumentation
  ...
  def via(uuid),
    do: {:via, Registry, {VialKeeper.Runtime.DatabaseRegistry, {:derived_worker, uuid}}}

  @spec pid(binary()) :: {:ok, pid()} | :error
  def pid(uuid) when is_binary(uuid) do
    case Registry.lookup(VialKeeper.Runtime.DatabaseRegistry, {:derived_worker, uuid}) do
      [{pid, _}] -> {:ok, pid}
      [] -> :error
    end
  end
  ...
  defp retry_delay(state, attempt) do
    ...
    ceiling = VialKeeper.Config.host_limits()[:max_materialized_view_retry_delay_ms] || 300_000
  ...
  defp batch_limit(definition) do
    ceiling = VialKeeper.Config.host_limits()[:max_materialized_view_batch_documents] || 500
  ...
  defp max_concurrent_sources(state) do
    ceiling = VialKeeper.Config.host_limits()[:max_materialized_view_concurrent_sources] || 16
