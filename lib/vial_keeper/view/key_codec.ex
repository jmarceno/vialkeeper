defmodule VialKeeper.View.KeyCodec do
  @moduledoc """
  ...
  @spec encode(key()) :: {:ok, binary()} | {:error, VialKeeper.Error.t()}
  ...
  def encode(_),
    do: {:error, VialKeeper.Error.invalid_request("view key must be an array")}
  ...
  @spec group_prefix(key(), non_neg_integer()) :: {:ok, binary()} | {:error, VialKeeper.Error.t()}
  def group_prefix(key, group_level)
    ...
    if group_level > length(key) do
      {:error, VialKeeper.Error.invalid_request("group_level exceeds key length")}
    else
  ...
  defp encode_component(value) when is_number(value) do
    case Number.to_binary64(value) do
      ...
      :overflow ->
        {:error, VialKeeper.Error.invalid_request("view key number must be finite")}
    end
  end
  ...
  defp encode_component(value) when is_binary(value) do
    if String.valid?(value) do
      ...
      {:error, VialKeeper.Error.invalid_request("view key string must be valid UTF-8")}
    end
  end

  defp encode_component(_),
    do: {:error, VialKeeper.Error.invalid_request("view key component type is not supported")}
