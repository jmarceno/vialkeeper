defmodule VialKeeper.View.Reducer do
  ...
  @spec normalize_reducer(term()) :: {:ok, reducer()} | {:error, VialKeeper.Error.t()}
  ...
  def normalize_reducer(_),
    do: {:error, VialKeeper.Error.invalid_request("view reducer is invalid")}

  @spec reduce_rows([row()], reducer()) :: {:ok, [map()]} | {:error, VialKeeper.Error.t()}
  ...
  @spec fold_values([term()], reducer()) :: {:ok, term()} | {:error, VialKeeper.Error.t()}
  ...
  defp collect_binary64(number, {:ok, acc}) when is_number(number) do
    case Number.to_binary64(number) do
      ...
      :overflow ->
        {:halt, {:error, VialKeeper.Error.resource_limit("numeric value is not representable")}}
    end
  end
