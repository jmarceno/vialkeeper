defmodule VialKeeper.View.Manager do
  @moduledoc """
  ...
  @spec start_builder(binary(), binary()) :: :ok | {:error, VialKeeper.Error.t()}
  def start_builder(uuid, view_id) when is_binary(uuid) and is_binary(view_id) do
    call(uuid, {:start_builder, view_id})
  end

  @spec start_builder(binary(), binary(), keyword()) :: :ok | {:error, VialKeeper.Error.t()}
  ...
  @spec await_resumed(binary()) :: :ok | {:error, VialKeeper.Error.t()}
  def await_resumed(uuid) when is_binary(uuid), do: call(uuid, :await_resumed)

  @spec stop_builder(binary(), binary()) :: :ok | {:error, VialKeeper.Error.t()}
  ...
  defp resume_persisted_builders(state) do
    case maintenance(state.uuid, {:command, :list_views, %{}}) do
      ...
      {:error, %VialKeeper.Error{code: :database_overloaded}} ->
