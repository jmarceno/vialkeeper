defmodule VialKeeper.View.NumericAccumulator do
  @moduledoc """
  ...
  @spec add(t(), number() | nil) :: {:ok, t()} | {:error, VialKeeper.Error.t()}
  ...
  def add(acc, value) when is_number(value) do
    case Number.to_binary64(value) do
      ...
      :overflow ->
        {:error, VialKeeper.Error.resource_limit("numeric value is not representable")}
    end
  end

  @spec remove(t(), number() | nil) :: {:ok, t()} | {:error, VialKeeper.Error.t()}
  ...
  @spec sum(t()) :: {:ok, float()} | {:error, VialKeeper.Error.t()}
  def sum(acc), do: units_to_binary64(acc.sum_units, @sum_scale)

  @spec sumsqr(t()) :: {:ok, float()} | {:error, VialKeeper.Error.t()}
  ...
  defp units_to_binary64(units, scale) when is_integer(units) do
    ...
    case round_units_to_binary64(absolute, scale, negative) do
      ...
      :resource_limit ->
        {:error, VialKeeper.Error.resource_limit("numeric aggregate is not representable")}
    end
  end
