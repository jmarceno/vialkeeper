defmodule VialKeeper.View.Definition do
  ...
  @spec normalize(map()) :: {:ok, t()} | {:error, VialKeeper.Error.t()}
  ...
  def normalize(_),
    do: {:error, VialKeeper.Error.invalid_request("view definition must be an object")}
  ...
  defp known_fields(definition) do
    if Enum.all?(Map.keys(definition), &(&1 in @known_fields)),
      do: :ok,
      else: {:error, VialKeeper.Error.invalid_request("view definition contains an unknown field")}
  end

  defp normalize_name(name) when is_binary(name) and name != "" do
    if byte_size(name) <= @max_name_bytes and String.valid?(name),
      do: {:ok, name},
      else: {:error, VialKeeper.Error.invalid_request("view name is invalid")}
  end

  defp normalize_name(_),
    do: {:error, VialKeeper.Error.invalid_request("view name is required")}
  ...
  defp normalize_selector(definition) do
    ...
    if disallowed != [] do
      {:error, VialKeeper.Error.invalid_request("view selector cannot include query controls")}
    else
  ...
  defp normalize_value(_value, _reducer),
    do: {:error, VialKeeper.Error.invalid_request("view value expression is invalid")}
  ...
  defp validate_value_requirement(_reducer, _value),
    do: {:error, VialKeeper.Error.invalid_request("view value is required for this reducer")}
