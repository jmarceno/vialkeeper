defmodule VialKeeper.Application do
  ...
  def start(_type, _args) do
    ...
    children = [
      ...
      {Task.Supervisor, name: VialKeeper.TaskSupervisor},
      VialKeeper.Runtime.DatabaseCatalog,
      VialKeeper.Shadow.Supervisor,
      VialKeeper.DerivedView.Supervisor,
      VialKeeper.DerivedView.Manager,
      http_server
    ]
  ...
  def listener_safety_error(listener) do
    ...
    if loopback?(ip) do
      ...
      auth = Application.get_env(:vial_keeper, :auth, [])
      tls = Application.get_env(:vial_keeper, :tls, [])
      security = Application.get_env(:vial_keeper, :security, [])
      ...
      if auth_enabled or tls_enabled or allow_insecure do
        ...
        {:error,
         """
         VialKeeper refuses to start on a non-loopback interface without
  ...
  defp http_server_child_spec(listener) do
    ...
    if Keyword.get(tls, :enabled, false) do
      ...
      Bandit.child_spec(
        [plug: VialKeeper.HTTP.Router, scheme: :http, http_options: http_options] ++ listener
      )
    end
  end
end
