defmodule VialKeeper.WebUI.Routes.Databases do
  @moduledoc """
  ...
  def create(conn) do
    with {:ok, params, conn} <- Request.fetch_params(conn),
      ...
      conn
      |> Response.put_hx_trigger(%{"vialkeeper:databases-changed" => %{}})
  ...
  defp create_form do
    [
      ...
      Components.field(
        ...
        [
          " <input name=\"path\" type=\"text\" autocomplete=\"off\" spellcheck=\"false\" placeholder=\"optional.vialkeeper\">"
        ]
      ),
  ...
  defp create_path(nil), do: {:ok, "#{VialKeeper.UUID.v4()}.vialkeeper"}
  defp create_path(""), do: {:ok, "#{VialKeeper.UUID.v4()}.vialkeeper"}
