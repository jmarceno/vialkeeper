defmodule VialKeeper.WebUI.Routes.Replications do
  @moduledoc """
  Replication job console fragments for a single database.

  Handlers call `VialKeeper.Replication.JobManager` for list/create and lifecycle
  ...
  def status(conn) do
    with {:ok, uuid} <- Request.require_uuid(conn.path_params["uuid"]),
         ...
         :ok <- VialKeeper.HTTP.Request.validate_path_id(job_id),
