defmodule VialKeeper.WebUI.Response do
  @moduledoc """
  HTML response helpers for the embedded Web UI.

  UI fragments use `text/html` and defensive browser headers. They intentionally
  avoid the machine JSON envelope owned by `VialKeeper.HTTP.Response`.
  """

  import Plug.Conn

  alias VialKeeper.Error
  alias VialKeeper.WebUI.Components
  alias VialKeeper.WebUI.HTML
  ...
  @doc """
  Renders a safe HTML error fragment from an `VialKeeper.Error`.
  """
  ...
  defp maybe_auth_off_header(conn) do
    ...
    if Keyword.get(auth, :enabled, false) do
      ...
      put_resp_header(conn, "x-vialkeeper-auth", "off")
    end
  end
end
