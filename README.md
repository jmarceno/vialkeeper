...
supervised Elixir application. Each registered database is a self-contained
`.elixirdb` bundle containing `database.sqlite3`, `blobs/` for immutable
content-addressed attachment bytes, and `tmp/` for non-authoritative temporary
files. Documents, complete revision histories, tombstones, changes, local
replication metadata, attachment manifests, and logical indexes live in the
bundle.
...
[docs/operations.md](docs/operations.md#authentication-and-tls)).
...
# Create (and open) a database bundle under the configured root.
...
# Close when finished; the bundle remains registered until unregister/1.
...
mutation. Downloads stream the original bytes:

```typescript
const upload = await fetch(
  `${baseUrl}/v1/databases/${uuid}/attachments/upload`,
...
await postJson(`/v1/databases/${uuid}/documents/put`, {
  id: "note-1",
  body: { title: "Hello" },
  attachments: { "source.txt": { blob: blob.blob, content_type: "text/plain" } },
});

const download = await fetch(
  `${baseUrl}/v1/databases/${uuid}/attachments/get`,
  { method: "POST", headers: { "content-type": "application/json" },
    body: JSON.stringify({ id: "note-1", revision: null, name: "source.txt" }) },
);
```

In-process callers use `ElixirDB.Attachments.upload_stream/2` and
`ElixirDB.Attachments.open_stream/2`; `ElixirDB.Documents.put/2` accepts the
same attachment reference map.

## Offline portability

Stop writes, close the database, and copy the complete closed `.elixirdb`
directory with ordinary operating-system tools. The bundle remains inert until
it is explicitly registered at the destination. Copy `database.sqlite3`,
`blobs/`, and `tmp/` together; a transient `.lease` companion is not
...
routing-only registrations.json manifest. Existing bundles are admitted only
through POST /v1/registrations with a relative path; registration validates
the SQLite markers, schema, UUID, physical bundle components, and integrity
before routing traffic to it. Unregistering removes routing metadata but never
deletes the bundle.

For an offline copy, stop writes, close the database with
POST /v1/databases/:uuid/close, copy the complete `.elixirdb` directory, and
register the bundle at the destination. A `.lease` file is transient ownership
state and is not authoritative database data.

## Replication jobs and limits

Replication job definitions, checkpoints, revisions, tombstones, changes,
attachment manifests, and logical indexes are stored in `database.sqlite3`.
Attachment bytes are stored in `blobs/`; runtime worker state is transient.
Enabled continuous jobs resume after restart; one-shot workers end
...
SQLite metadata and physical attachment state.
